...
$partitionMapEntrySignature = $partitionMapSignature;
...
sub WriteFile
{
    ...
    $result = write($file_handle, $$buffer, $number_of_bytes_to_read);
...
sub SeekToBlock
{
    ...
    SeekTo($file_handle, $offset * $block_size);
}
...
sub Main
{
	...
	for $partition_map_entry (@partition_map)
	{
        ...
        print($partition_name . ' ' . $size . ' blocks' . $newline);
        
        $partition_file_handle = OpenBinaryFileForWrite($partition_name);
        SeekToBlock($file_handle, $start);
        while ($size != 0)
        {
            ReadFile($file_handle, \$buffer, $block_size);
            WriteFile($partition_file_handle, \$buffer, $block_size);
            $size -= 1;
        }  
	}
}
