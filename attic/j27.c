...
static const char sourcefile[] = "j27.c";
...
{
	jk_unused((&buffer_size, &buffer, &data));
...
{
	jk_unused((&t, &buffer_size, &buffer, &data));
}
    ...
    )
{
    ...
    for ( offset = offset_to_first_field ; offset <= offset_to_last_field ; offset += sizeof(jk_field_t) )
    {
		...
		{	
			...
			if (err < 0)
				goto exit;
			if (dump_length > buffer_size)
			{
				size_t new_buffer_size;
				
				free(malloced_buffer);
				new_buffer_size = (buffer_size * 2);
				if (new_buffer_size < dump_length) /* this catches overflow too */
				{
					new_buffer_size = dump_length;
				}
				malloced_buffer = (char*)malloc(new_buffer_size);
				if (malloced_buffer == 0 && new_buffer_size > dump_length)
				{
					new_buffer_size = dump_length;
					malloced_buffer = (char*)malloc(new_buffer_size);
				}
				if (malloced_buffer == 0)
				{
					err = jk_out_of_memory_error;
					goto exit;
				}
				buffer = malloced_buffer;
				buffer_size = new_buffer_size;
			}
			err = (*dump)(type, buffer_size, buffer, (base + field->unpacked_offset));
		}
		else
		{
			...
			if (u >= 16)
			{
				jk_printf("%s%-*s %u (0x%x)\n", prefix, (int)maximum_name_length, field->name, u, u);
			}
			else
			{
				jk_printf("%s%-*s %u\n", prefix, (int)maximum_name_length, field->name, u);
			}
		}
    }
exit:
	jk_free((void**)&malloced_buffer);
	return err;
}
...
#define jkp_type_integer_type(t,u,s) \
jk_type_t jk_type_##t = \
{ \
    sizeof(u), \
...
jkp_type_integer_type(uchar, unsigned char, jk_false)
jkp_type_integer_type(char, char, jk_true)
jkp_type_integer_type(short, short, jk_true)
jkp_type_integer_type(ushort, unsigned short, jk_false)
jkp_type_integer_type(int, int, jk_true)
jkp_type_integer_type(uint, unsigned, jk_false)
jkp_type_integer_type(long, long, jk_true)
jkp_type_integer_type(ulong, unsigned long, jk_false)
...
jk_type_t jk_type_struct;
jk_type_t jk_type_void_pointer;
	...
	)
{
	...
	void * new_bytes = jk_pointer_uninitialized;
	...
	if (buffer->static_size >= size)
	{
		buffer->bytes = buffer->static_buffer;
		goto success;
	}
	allocated_size *= 2;
	if (allocated_size < size)
	...
	err = allocator->functions->allocate(allocator, allocated_size, &new_bytes);
