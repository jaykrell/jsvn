...
static const char sourcefile[] = "j51.c";
    ...
    )
{
    ...
    union {
    	jk_ulong_t i;
    	unsigned char c[sizeof(jk_ulong_t)];
    } uhost_endian = { 1 };
    ...
    for ( offset = offset_to_first_field ; offset <= offset_to_last_field ; offset += sizeof(field) )
    {
		...
		{
    		jk_field_unpacker_t* field_unpacker = *(jk_field_unpacker_t **)(((jk_uchar_t *)&jk_field_unpackers) + field.unpacker_offset);
...
{
	jk_unused(&data);
    return JK_INTEGER_DUMP_LENGTH(jk_ulong_t);
}
...
{
	jk_unused(&t);
	*length = jk_get_dump_length_unsigned_int(*(const jk_uint_t*)data);
	return 0;
}
...
{
	jk_unused(&t);
	*length = jk_get_dump_length_unsigned_long(*(const jk_ulong_t*)data);
	return 0;
}
...
{
    jk_dump_unsigned_long(buffer_size, buffer, (jk_ulong_t)data);
}
...
void jk_dump_long(size_t buffer_size, char * buffer, long data)
{
    ...
    if (data >= 16)
        jk_sprintf(big_enough_buffer, "%ld (0x%lx)", data, (jk_ulong_t)data);
    else
...
{
	char big_enough_buffer[JK_INTEGER_DUMP_LENGTH(jk_ulong_t)];
...
{
	jk_unused(&t);
	jk_dump_unsigned_int(buffer_size, buffer, *(const jk_uint_t *)data);
	return 0;
}
...
{
	jk_unused(&t);
	jk_dump_unsigned_long(buffer_size, buffer, *(const jk_ulong_t *)data);
	return 0;
}
...
jk_uint_t jk_get_big_endian_u2(const jk_uchar_t * a)
{
    return (((jk_uint_t)a[0]) << 8) | (jk_uint_t)a[1];
}

jk_ulong_t jk_get_big_endian_u4(const jk_uchar_t * a)
{
    return (((jk_ulong_t)a[0]) << 24)
    		| (((jk_ulong_t)a[1]) << 16)
    		| (((jk_ulong_t)a[2]) << 8)
    		| (jk_ulong_t)a[3]
    		;
...
jk_uint_t jk_get_little_endian_u2(const jk_uchar_t * a)
{
    return (((unsigned)a[1]) << 8) | a[0];
}

jk_ulong_t jk_get_little_endian_u4(const jk_uchar_t * a)
{
    return (((jk_ulong_t)a[3]) << 24)
    		| (((jk_ulong_t)a[2]) << 16)
    		| (((jk_ulong_t)a[1]) << 8)
    		| (jk_ulong_t)a[0]
    		;
...
jk_unsigned_add_with_carry_in(						jk_ulong_add_with_carry_in, jk_ulong_t)
jk_unsigned_add_with_carry_out(						jk_ulong_add_with_carry_out, jk_ulong_t)
jk_unsigned_add_with_carry_in_and_out(				jk_ulong_add_with_carry_in_and_out, jk_ulong_t)
jk_unsigned_add_in_place_with_carry_in(				jk_ulong_add_in_place_with_carry_in, jk_ulong_t)
jk_unsigned_add_one_with_carry_out(					jk_ulong_add_one_with_carry_out, jk_ulong_t)
jk_unsigned_add_one_in_place_with_carry_out(		jk_ulong_add_one_in_place_with_carry_out, jk_ulong_t)
...
void
jk_ulong_add_in_place_with_carry_out(jk_ulong_t * a, jk_ulong_t b, jk_carry_t * carry)
{
    jk_ulong_add_with_carry_out(a, *a, b, carry);
}

void
jk_ulong_add_in_place_with_carry_in_and_out(jk_ulong_t * a, jk_ulong_t b, jk_carry_t * carry)
{
...
{
    jk_ulong_t * malloced_value_array;
    jk_ulong_t built_in_value_array[4];
    unsigned precision;
    ...
    )
{
	...
	{
		memcpy(m->built_in_value_array, m->malloced_value_array, (m->precision * sizeof(jk_ulong_t)));
...
{
    const jk_ulong_t * value_array = jk_multiprecision_integer_get_value_array(m);
...
{
    jk_ulong_t * value_array = jk_multiprecision_integer_get_value_array(m);
    unsigned precision = m->precision;    
    for ( ; precision != 0 ; (--precision), ++value_array)
    {
        *value_array ^= ~0UL;
...
{
    jk_ulong_t * value_array = jk_multiprecision_integer_get_value_array(m);
    const jk_ulong_t * value_array2 = jk_multiprecision_integer_get_value_array(n);
    unsigned precision = m->precision;
    const unsigned precision2 = n->precision;
    jk_ulong_t any_zero = 0;
    jk_ulong_t t;
    unsigned p;
...
{
    jk_ulong_t * value_array = 0;
    const jk_ulong_t * value_array2 = jk_multiprecision_integer_get_value_array(m2);
    ...
    )
{
	...
	err = jk_malloc((void**)&newp, sizeof(jk_ulong_t) * new_precision);
	if (err < 0)
		return err;

    oldp = jk_multiprecision_integer_get_value_array(m);
    memcpy(newp, oldp, old_precision * sizeof(jk_ulong_t));
    memset(newp + old_precision, 0, (new_precision - old_precision) * sizeof(jk_ulong_t));
    jk_free((void**)&m->malloced_value_array);
...
jk_ulong_t
jk_ulong_extract_left_n_bits_right_aligned(jk_ulong_t a, unsigned n)
{
	const unsigned k = jk_bits_of(jk_ulong_t);
    return (a >> (k - n));
}

jk_ulong_t
jk_ulong_extract_left_n_bits(jk_ulong_t a, unsigned n)
{
    return (a & ((~0UL) << n));
}

jk_ulong_t
jk_ulong_extract_right_n_bits(jk_ulong_t a, unsigned n)
{
    return (a & ~((~0UL) << n));
}

jk_ulong_t
jk_ulong_extract_right_n_bits_left_aligned(jk_ulong_t a, unsigned n)
{
	const unsigned k = jk_bits_of(jk_ulong_t);
    return (a << (k - n));
}
...
{
	return (long)(((jk_ulong_t)a) >> shift);
}
...
jk_ulong_t jk_ulong_with_n_right_bits_clear(unsigned n) { return        ((~0UL) << n); }
jk_ulong_t jk_ulong_with_n_right_bits_set(unsigned n)   { return       ~((~0UL) << n); }
jk_ulong_t jk_ulong_with_n_left_bits_clear(unsigned n)  { return        ((~0UL) >> n); }
jk_ulong_t jk_ulong_with_n_left_bits_set(unsigned n)    { return       ~((~0UL) >> n); }
...
jk_ulong_t
jk_ulong_one_fill_right_shift(jk_ulong_t a, unsigned shift)
{
	const unsigned k = jk_bits_of(jk_ulong_t);
...
jk_ulong_t
jk_ulong_zero_fill_right_shift(jk_ulong_t a, unsigned shift)
{
	return (a >> shift);
}
...
jk_ulonglong_right_rotate(jk_ulonglong_t * a, const jk_ulonglong_t * b, unsigned shift)
{
	const unsigned k = jk_bits_of(jk_ulong_t);
	const unsigned k2 = (k * 2);
	const jk_ulong_t b_high = b->high;
	const jk_ulong_t b_low = b->low;	
	const unsigned shift_m_k2 = (shift % k2);
	const unsigned shift_m_k = (shift % k);
	jk_ulong_t a_low  = (b_high >> shift_m_k) | jk_ulong_extract_right_n_bits_left_aligned(b_low, shift_m_k);
	jk_ulong_t a_high = ( b_low >> shift_m_k) | jk_ulong_extract_right_n_bits_left_aligned(b_high, shift_m_k);
	if (shift_m_k2 >= k)
	{
		jk_ulong_t t = a_low;
		a_low = a_high;
...
jk_ulonglong_left_rotate(jk_ulonglong_t * a, const jk_ulonglong_t * b, unsigned shift)
{
	...
	const unsigned shift_m_k2 = (shift % k2);
	const unsigned shift_m_k = (shift % k);
	jk_ulong_t a_low  = (b_high << shift_m_k) | jk_ulong_extract_left_n_bits_right_aligned(b_low, shift_m_k);
	jk_ulong_t a_high = ( b_low << shift_m_k) | jk_ulong_extract_left_n_bits_right_aligned(b_high, shift_m_k);
	if (shift_m_k2 >= k)
	{
		jk_ulong_t t = a_low;
		a_low = a_high;
...
jk_ulonglong_one_fill_right_shift(jk_ulonglong_t * a, const jk_ulonglong_t * b, unsigned shift)
{
	const unsigned k = jk_bits_of(jk_ulong_t);
	const unsigned k2 = (k * 2);
	jk_ulong_t a_low = ~0UL;
	jk_ulong_t a_high = ~0UL;
	const jk_ulong_t b_high = b->high;
    if (shift < k2)
    {
        if (shift >= k)
...
jk_ulong_t jk_ulong_get_right_justified_high_half(jk_ulong_t a)
{
    return (a >> (jk_bits_of(a) / 2));
}

jk_ulong_t jk_ulong_get_low_half(jk_ulong_t a)
{
    return  (a & (~0UL >> (jk_bits_of(a) / 2)));
}

jk_ulong_t jk_ulong_get_left_justified_high_half(jk_ulong_t a)
{
    return  (a & (~0UL << (jk_bits_of(a) / 2)));
}

void
jk_ulong_multiply_to_ulonglong(jk_ulonglong_t* a, jk_ulong_t b, jk_ulong_t c)
{
/*
ab * cd = a * d * 10 + b * c * 10 + b * d + a * c * 100
*/

    const jk_ulong_t bl = jk_ulong_get_low_half(b);
    const jk_ulong_t bh = jk_ulong_get_right_justified_high_half(b);
    const jk_ulong_t cl = jk_ulong_get_low_half(c);
    const jk_ulong_t ch = jk_ulong_get_right_justified_high_half(c);
    jk_ulonglong_t aa[2];
    jk_carry_t carry;
    ...
    aa[1].high = carry;    
    jk_ulonglong_left_shift_in_place(&aa[1], jk_bits_of(jk_ulong_t) / 2);
    ...
    )
{
    ...
    magnitude->high = (jk_ulong_t)local_high;
    *sign = local_sign;
}
...
{
    const jk_ulong_t bl = b->low;
    const jk_ulong_t bh = b->high;
    const jk_ulong_t cl = c->low;
    const jk_ulong_t ch = c->high;
    jk_ulonglong_t t;
    ...
    )
{
    ...
    jk_ulong_t mantissa_low32;
    jk_ulong_t mantissa_high20;
    double result;
...
{
	i->high =  (((jk_ulong_t)p[0]) << 24)
            | (((jk_ulong_t)p[1]) << 16)
            | (((jk_ulong_t)p[2]) << 8)
            | ((jk_ulong_t)p[3])
            ;
    i->low =   (((jk_ulong_t)p[4]) << 24)
            | (((jk_ulong_t)p[5]) << 16)
            | (((jk_ulong_t)p[6]) << 8)
            | ((jk_ulong_t)p[7])
            ;
...
#define X(x) offsetof(local_t, x)
    ...
    JK_STATIC_CONST struct {
        ...
        { X(name_uint), jk_constant_string("unsigned int") },
	...
	)
{
	...
	if (d->bits_per_pixel < 8)
	{
		unsigned a = (x % 8);
		unsigned b = *p;
		*p = ((b & d->plot_bit[a].mask) | (bit ? d->plot_bit[a].set : 0));
	}
	else
	{
...
typedef struct jk_appleiigs_languagecard_t
{
	unsigned char bytes[16384];
	unsigned char* banks[64];
} jk_appleiigs_languagecard_t;

typedef struct jk_appleiigs_t
{
	unsigned char code;
	unsigned pc_page;
	unsigned a;
	...
	unsigned char*  shadow_memory_pp[512];

	unsigned char* rom_pointers[4][256];
	unsigned char* bank0_pointers[256];
	unsigned char* bank1_pointers[256];
	unsigned char* banke0_pointers[256];
	unsigned char* banke1_pointers[256];
	unsigned char invalid_page[256];
	unsigned char* invalid_bank[256];
	jk_appleiigs_languagecard_t language_card0;
	jk_appleiigs_languagecard_t language_card1;
} jk_appleiigs_t, jk_iigs_t;
...
void
jk_iigs_init_language_card(
	jk_appleiigs_languagecard_t* p
	)
{
	unsigned i;
	for (i = 0 ; i != 64 ; ++i)
	{
		p->banks[i] = &p->bytes[i * 256];
	}
}
	...
	)
{
	...
	pp = (unsigned char**)calloc(256, sizeof(unsigned char*));
	if (pp == NULL)
	{
		goto Exit;
	}
	iigs->memory_ppp[i] = pp;
	p = &iigs->memory_p[i * 256UL * 256UL];
	for (j = 0 ; j != 256 ; ++j)
	{
		pp[j] = &p[j * 256];
	}
...
typedef typedef struct jk_iigs_memory_details
{
	unsigned char *native_read;
	unsigned char *native_write;
	unsigned char *native_write_shadow;
	void (*read_function)();
	void (*write_function)();
} jk_iigs_memory_details;
...
jk_iigs_analyze_address(
	...
	jk_iigs_memory_details* details
	)
{
	unsigned char* native;
	unsigned bank;
	unsigned offset_within_bank;

	bank = (address >> 16);
	offset_within_bank = (address & 0xFFFF);

	details->native_write_shadow = NULL;
	details->read_function = NULL;
	details->write_function = NULL;
	
	if (bank >= 2 && bank < 0x80)
	{
		native = &iigs->memory[address & iigs->ghost];
		details->native_read = native;
		details->native_write = native;
		return;
	}
	else if (bank >= 0xE1 && bank < 0xFC)
	{
		native = &iigs->bit_bucket_page[address & 0xFF];
		details->native_read = native;
		details->native_write = native;
		return;
	}
	...
	{
		details->native_read  = &iigs->rom[address & 0x3FFFF];
		details->native_write = &iigs->bit_bucket_page[address & 0xFF];
		return;
	}
	else if (bank == 0)
	{
		details->native_read = &iigs->bank[offset_within_page];
	}
}

void
jk_appleiigs_free_memory_bank(
	jk_iigs_t* iigs,
	unsigned i
	)
{
...
void
jk_appleiigs_init_invalid_memory_bank(
	)
{
}
	...
	)
{
	...
	for (i = 0 ; i != 512 ; ++i)
	{
		iigs->shadow_memory[i] = NULL;
	}
	iigs->memory_p = NULL;
	iigs->memory_ppp = NULL;

	p = (unsigned char*)malloc(256UL * 256UL * 256UL); /* a full 16meg */
	if (p == NULL)
		goto Exit;
	iigs->memory_p = p;
	ppp = (unsigned char***)calloc(256, sizeof(unsigned char**)); /* 256 bank pointers */
	if (ppp == NULL)
		goto Exit;
	iigs->memory_ppp = ppp;
	for (i = 0 ; i != 256 ; ++i)
	...
	for (i = 0 ; i != 0x10 ; ++i)
	{
		if (!jk_appleiigs_init_memory_bank(i))
			goto Exit;
	}
	if (!jk_appleiigs_init_memory_bank(0xE0))
		goto Exit;
	if (!jk_appleiigs_init_memory_bank(0xE1))
		goto Exit;
	if (!jk_appleiigs_init_memory_bank(0xFC))
		goto Exit;
	if (!jk_appleiigs_init_memory_bank(0xFD))
		goto Exit;
	if (!jk_appleiigs_init_memory_bank(0xFE))
		goto Exit;
	if (!jk_appleiigs_init_memory_bank(0xFF))
		goto Exit;
	for (i = 0x10 ; i != 0x80 ; ++i)
	{
		if (!jk_appleiigs_init_ghost_memory_bank(i))
			goto Exit;
	}
	for (i = 0x80 ; i != 0xE0 ; ++i)
	{
		if (!jk_appleiigs_init_invalid_memory_bank(i))
			goto Exit;
	}
	for (i = 0xE2 ; i != 0xFC ; ++i)
	{
	...
	for (i = 0 ; i != 4 ; ++i)
	{
		for (j = 0 ; i != 256 ; ++i)
		{
			iigs->rom_pointers[i][j] = &iigs->rom[i][j << 8];
		}
	}
	for (i = 0 ; i != 256 ; ++i)
	{
		iigs->invalid_bank[i] = &iigs->invalid_page[0];
		iigs->bank0[i] = ppp[0][i];
		iigs->bank1[i] = ppp[1][i];
		iigs->banke0[i] = ppp[0xe0][i];
		iigs->banke1[i] = ppp[0xe1][i];
	}	
	success = 1;
Exit:
	if (!success)
	{
		for (i = 0 ; i != 256 ; ++i)
		{
			jk_appleiigs_free_memory_bank(iigs, i);
		}
		free(iigs->memory_ppp);
		iigs->memory_ppp = NULL;
		free(iigs->memory_p);
		iigs->memory_p = NULL;
	}
	...
	switch (address & 0xFFF)
	{
		...
		case 0x088: case 0x089: case 0x08A: case 0x08B: case 0x08C: case 0x08D: case 0x08E: case 0x08F:
			...
			switch (address & 0xB) /* 1011 */
			{
				...
				case 0x8: /* 1000 */
				case 0xB: /* 1011 */
					jk_iigs_io_language_card_read_ram(iigs);
					break;
				case 0x1: /* 0001 */
				case 0x2: /* 0010 */
				case 0x9: /* 1001 */
				case 0xA: /* 1010 */
					jk_iigs_io_language_card_read_rom(iigs);
					break;
			}
			switch (address & 1)
			{
				case 0x0:
					jk_iigs_io_language_card_write_protect_ram(iigs);
					break;
				case 0x1:
					if  ((previous & 0xFF1) == 0x081)
					{
						jk_iigs_io_language_card_write_enable_ram(iigs);
						iigs->io_previous = 0;
					}
					break;
			}
			jk_iigs_io_select_language_card_bank(iigs, (address & 8) ? 1 : 0);
			break;
	}
	return value;
#endif
	...
	)
{
	...
	case 0xE0: return jk_appleiigs_memory_handler_bank_0xE0;
	case 0xE1: return jk_appleiigs_memory_handler_bank_0xE1;
...
jk_appleiigs_memory_handler_t* jk_appleiigs_memory_handler_from_address(unsigned long address)
{
	return jk_appleiigs_memory_handler_from_bank(((unsigned)(address >> 16));
}
...
enum jk_iigs_instruction_addressing_mode
{
	immediate, /* lda #1, pea */
	accumulator, /* inc a */
...
static
long
_jk_buffer_stdio_flush_or_fill(
	void* context,
	jk_byte_t* memory,
	size_t number_of_bytes,
	size_t* out_number_of_bytes,
	size_t (*read_or_write)(p, size, count, file)
	)
{
	int error = 0;
	...
	{
		size_t n = (*read_or_write)((FILE*)context, 1, number_of_bytes, memory);
