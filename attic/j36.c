...
static const char sourcefile[] = "j36.c";
...
#if jk_debug_strings
#define jk_profile_function_name_string jk_constant_string
/* This crashes PPCC in C mode.
define jk_profile_function_name_string(x) jk_constant_string(x)
*/
#else
#define jk_profile_function_name_string(x) jk_constant_string("")
#endif
...
#define jk_profile_count_function_call(function_name) \
	do { \
		static jk_profile_record_t profile_record = { 0, 0, { jk_profile_function_name_string(#function_name) } }; \
    ...
    )
{
	...
	jk_profile_count_function_call(jk_profile_dump_function_call_counts);
	...
	)
{
	...
	*pp = NULL;
	if (p != NULL)
	{
		jk_profile_count_function_call(jk_free_non_null);
		free(p);
	}
}
...
{
	jk_profile_count_function_call(jk_default_memory_allocator_convert_cookie_to_pointer);
...
#if 0
	jk_printf(jk_format_file_line "%s bytes_to_read /* %u */ /* %u */\n", sourcefile, (unsigned)__LINE__, function, (unsigned)bytes_to_read, (unsigned)n);
#endif
    return 0;
}
    ...
    )
{
	...
	jk_profile_count_function_call(jk_limited_truncating_string_copy);
    ...
    )
{
	...
	jk_profile_count_function_call(jk_dump_struct);
...
/*jk_unsigned_add_in_place_with_carry_out(			jk_ulong_add_in_place_with_carry_out, ulong_t)*/
/*jk_unsigned_add_in_place_with_carry_in_in_and_out( jk_ulong_add_in_place_with_carry_in_and_out, ulong_t)*/
    ...
    )
{
    ...
    p = (ulong_t*)malloc(sizeof(ulong_t) * n);
    if (p == NULL)
...
{
    int local_sign = 1;
    ulong_t local_magnitude = (ulong_t)i;
    if (i < 0)
    {
        local_magnitude = ((ulong_t)-(i + 1)) + 1;
        local_sign = -1;
    }
    *sign = local_sign;
    ...
    )
{
	...
	jk_profile_count_function_call(jk_ltoa);
    ...
    )
{
	...
	jk_profile_count_function_call(jk_itoa);
    ...
    )
{
    ...
    jk_unused(function); 
	jk_profile_count_function_call(jk_java_convert_java_double_to_native_double);
    ...
    )
{
	...
	jk_profile_count_function_call(jk_java_convert_java_float_to_native_float);
    jk_unused(function); 
    ...
    )
{
	...
	jk_profile_count_function_call(jk_java_read_constant_pool);
	...
	)
{
	...
	jk_profile_count_function_call(jk_init_file_buffer);
...
{
	jk_profile_count_function_call(jk_free_file_buffer);
	...
	)
{
	...
	jk_profile_count_function_call(jk_init_file);
...
{
	jk_profile_count_function_call(jk_free_file);
	jk_file_close(file);
	...
	)
{
	...
	jk_profile_count_function_call(jk_java_init_constant_pool);
	x->count = 0;
	x->tags = 0;
	x->any = 0;
	return 0;
	...
	)
{
	...
	jk_profile_count_function_call(jk_java_init_classfile);
...
{
	jk_java_classfile_t * const x = classfile;
	jk_profile_count_function_call(jk_java_free_classfile);
    ...
    )
{
	...
	jk_profile_count_function_call(jk_java_read_classfile);
