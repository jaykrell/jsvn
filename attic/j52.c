...
unsigned
jk_appleiigs_init_memory_bank(unsigned i)
{
	unsigned char* p;
	unsigned char** pp;
	unsigned j;
...
void
jk_appleiigs_free_memory_bank(unsigned i)
{
	unsigned char** pp;
	unsigned char*** ppp;
	unsigned j;
...
jk_appleiigs_init_memory()
{
	...
	p = (unsigned char*)malloc(256 * 256 * 256); /* a full 16meg */
	if (p == NULL)
		goto Exit;
	iigs->memory_p = p;
	...
	)
{
	...
	read = (read_or_write == jk_iigs_io_is_read);
	write = (read_or_write == jk_iigs_io_is_write);
...
void jk_appleiigs_io_write(unsigned adddress, unsigned value)
{
}

jk_appleiigs_memory_handler_t* jk_appleiigs_memory_handler_from_bank(unsigned bank)
{
	switch (bank)
	{
...
void jk_appleiigs_fetch_code_bytes1(iigs_t* iigs)
{
	if ((kpc & 0xff) == 0)
	{
		unsigned pc_page = (pc >> 8);
		kpc_pointer = &memory_data[k][pc_page][pc & 0xff];
	}
	code_byte = *kpc_pointer;
	kpc_pointer += 1;
	kpc += 1;
	pc += 1;
}

#define jk_appleiigs_fetch_code_bytes2(code_bytes) \
{ \
	if ((kpc & 0xff) < 0xfe) \
	{ \
		code_byte = kpc_pointer[1]; \
		code_byte <<= 8;
		code_byte |= kpc_pointer[0]; \
		kpc_pointer += 2;
	} \
	else \
	{
...
void jk_appleiigs()
{
	...
	unsigned long cycles;
	unsigned k_cache;
	unsigned char* memory_cache;

	jk_appleiigs_fetch_code_bytes1(opcode);
	
	if ((kpc & 0xff) == 0)
	{
		unsigned pc_page = (pc >> 8);
		kpc_pointer = &memory_data[k][pc_page];
	}
	opcode = *kpc_pointer;
	kpc_pointer += 1;
	kpc += 1;
