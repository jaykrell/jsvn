...
static const char sourcefile[] = "j49.c";
...
void* jk_cpu_x86 = "x86";
void* jk_cpu_powerpc = "powerpc";

void jkp_1();
void jkp_2(int i);
int jkp_3(int i);
int jkp_4(int i, int j);

void jkp_1() { printf(""); }
void jkp_2(int i) { printf("%d", i); }
int jkp_3(int i) { return printf("%d", i); }
int jkp_4(int i, int j) { int k = i + j; return printf("%d", k); }
 
void
jk_generate_code_prolog(
	jk_code_generater_t* cg,
	unsigned calling_convention,
	unsigned in_parameter_words,
	unsigned local_words,
	unsigned max_out_parameter_words,
	unsigned return_words
	)
{
	cg->processor = (void*)&jkp_1;
	cg->processor = (void*)&jkp_2;
	cg->processor = (void*)&jkp_3;
	cg->processor = (void*)&jkp_4;
	
	cg->calling_convention = calling_convention;
	cg->in_parameter_words = in_parameter_words;
	cg->local_words = local_words;
	cg->max_out_parameter_words = max_out_parameter_words;
	cg->return_words = return_words;
	cg->is_leaf = 0;
	
	if (cg->processor == &jk_cpu_x86)
	{
	/*
	push ebp
	sub ebp, local_words
	...
	*/
	...
	else if (cg->processor == &jk_cpu_powerpc)
	{
	}
}
...
jk_convert_character_bitmap_579_to_59array(
	(jk_character_bitmap_5x7x9_t *)&jk_character_bitmap_ABCDEFGHI,
...
jk_convert_character_bitmap_578_to_59array(
	(jk_character_bitmap_5x7x8_t *)&jk_character_bitmap_STUVWXYZ,
	&jk_character_bitmaps['S']);

jk_convert_character_bitmap_599_to_59array(
	(jk_character_bitmap_5x9x9_t *)&jk_character_bitmap_abcdefghi,
	&jk_character_bitmaps['a']);

jk_convert_character_bitmap_599_to_59array(
	(jk_character_bitmap_5x9x9_t *)&jk_character_bitmap_jklmnopqr,
	&jk_character_bitmaps['j']);

jk_convert_character_bitmap_598_to_59array(
	(jk_character_bitmap_5x9x8_t *)&jk_character_bitmap_stuvwxyz,
	&jk_character_bitmaps['s']);
...
jk_macos_release_display(&macos_display);
jk_macosx_release_display(&macosx_display);

	return -1;
}

void
jk_code_generater_generate_x86_load_register(
	jk_code_generater_t* cg,
	unsigned reg,
	jk_code_generater_value_t* value
	)
{
	unsigned same = (reg == value->registers[0]);
	if (value->indirect[0] == 0)
	{
 		if (value->offset[0] == 0)
 		{
 			if (value->registers[0] == reg)
 			{
 				return;
 			}
 			jk_code_generater_generate_x86_move_register_to_register(cg, value->registers[0], reg);
 			return;
 		}
 	}
}
...
void
jk_code_generater_generate_x86_push(
	jk_code_generater_value_t* value
	)
{
	unsigned scratch;
	if (value->indirect[0] == 0)
	{
 		if (value->offset[0] == 0)
 		{
 			jk_code_generater_generate_x86_push_register(value->registers[0]);
 			return;
 		}
 		else if (address->registers[0] == 0)
 		{
 			jk_code_generater_generate_x86_push_constant(value->offset[0]);
 			return;
 		}
	}
	scratch = jk_code_generater_get_scratch_register(cg);
	jk_code_generater_generate_x86_load_register(scratch, value);
	jk_code_generater_generate_x86_push_register(scratch);
	jk_code_generater_generate_x86_free_scratch(scratch);
}

unsigned
jk_code_generater_address_is_valid(
	jk_code_generater_address_t* a
	)
{
	unsigned operands = 0;
	unsigned indirections = 0;
	
	operands = (a->valid_r2 + a->valid_r3 + a->valid_n1);
	if (operands == 0)
		return 0;
	if (operands == 3)
		return 0;
	if (operands != a->operands)
		return 0;

	if (operands == 1 && a->indirect_sum)
		return 0;

	indirections = (a->indirect_n1 + a->indirect_r2 + a->indirect_r3 + a->indirect_sum);
	if (indirections > 2)
		return 0;
	if (indirections != a->indirections)
		return 0;

	if (a->valid_n1_constant && a->valid_n1_label)
		return 0;
	if (!a->valid_n1)
	{
		if (a->valid_n1_constant)
			return 0;
		if (a->valid_n1_label)
			return 0;
	}
