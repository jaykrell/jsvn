...
const static char sourcefile[] = "j14.c";
...
/*
i  = (high << 32) | low;
unsigned long might be more than 32 bits
*/
{
    /* make sure we never get more than 32bits per ulong (this check
       is rough, since we can't tell if people passed us zeros on purpose)
    */
    jk_assert((high >> 32) == 0);
    jk_assert((low >> 32) == 0);
/*
    if (jk_bits_of(unsigned long) == 32)
    {
        i->high = high;
        i->low = low;
    }
    ...
    )
{
    ...
    exponent = (ll_exponent.low & 0x7ff);
    ...
    mantissa_low32 = (ll_mantissa.low & 0xfffffffff);
    ...
    printf("jk_java_convert_java_double_to_native_double %f\n", result);
