...
sub Main
{
	...
	for $partition_map_entry (@partition_map)
	{
        ...
        print('name=' . $partition_name . ', type=' . $type . ', size=' . $size . ' blocks' . $newline);
        ...
        while ($size != 0)
        {
            ReadFile($file_handle, \$buffer, $block_size) || die('read failed:' . $!);
            WriteFile($partition_file_handle, \$buffer, $block_size) || die('read failed:' . $!);
            $size -= 1;
        }  
	}
}
