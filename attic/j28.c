...
static const char sourcefile[] = "j28.c";
    ...
    )
{
	...
	err = jk_init_buffer(&buffer, sizeof(stack_buffer), &stack_buffer);
	if (err < 0) goto exit;
    ...
    for ( offset = offset_to_first_field ; offset <= offset_to_last_field ; offset += sizeof(jk_field_t) )
    {
		...
		if (get_dump_length && dump)
		{
			...
			if (err < 0)
				goto exit;
			jk_sizet_add_one_in_place_with_carry_out(&dump_length, &carry);
			err = jk_carry_to_error(carry);
			if (err < 0)
				goto exit;
			err = jk_buffer_set_size(&buffer, dump_length, (void**)&p);
			if (err < 0)
				goto exit;
			jk_printf("%s%-*s\n", prefix, (int)maximum_name_length, field->name);
			err = (*dump)(type, dump_length, p, (base + field->unpacked_offset));
			jk_printf("\n");
		}
    }
exit:
	jk_free_buffer(&buffer);
	return err;
}
