...
typedef struct jk_appleiigs_t {
	...
	unsigned char (***io_read_byte)(struct jk_appleiigs_t* iigs, unsigned long ea);
	unsigned char (***write_byte)(struct jk_appleiigs_t* iigs, unsigned long ea, unsigned b);
	unsigned char** memory_data;
	unsigned long cycles;
	unsigned k_cache;
	unsigned char (*read_byte_cache)(struct jk_appleiigs_t* iigs, unsigned long ea);
	unsigned char (*write_byte_cache)(struct jk_appleiigs_t* iigs, unsigned long ea, unsigned b);
} jk_appleiigs_t;

unsigned char jk_appleiigs_read_byte_direct_bank[256](struct jk_appleiigs_t* iigs, unsigned long ea);

unsigned char jk_appleiigs_read_byte_direct(jk_appleiigs_t* iigs, unsigned long ea)
{
	unsigned bank = (ea >> 16);
	unsigned page = ((ea >> 8) & 0xff);
	unsigned offset = (ea & 0xff);
	return iigs->memory_data[bank][page][offset];
}

void jk_appleiigs_write_byte_direct(jk_appleiigs_t* iigs, unsigned long ea, unsigned b)
{
	unsigned bank = (ea >> 16);
	unsigned page = ((ea >> 8) & 0xff);
	unsigned offset = (ea & 0xff);
	iigs->memory_data[bank][page][offset] = (unsigned char)b;
}

void jk_appleiigs_init_memory()
{
	unsigned i = 0;
	for ( ; i != 256 ; ++i)
	{
		jk_appleiigs_read_byte_direct_bank[i] = &jk_appleiigs_read_byte_direct;
	}
}
