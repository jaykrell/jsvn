...
static const char sourcefile[] = "j50.c";
...
{
	unsigned d_bits_per_pixel_even_div_8 = d->bits_per_pixel_even_rounded_up_div_8;
	unsigned char* p = (x * d_bits_per_pixel_even_div_8 + y * d->bytes_per_row + d->base_address);
	...
	for (cy = 0 ; cy != c_height ; ++cy)
	{
		for (cx = 0 ; cx != c_width ; ++cx)
		{
			jkp_display_plot_bit(
				...
				x * 5 + cx,
				y * 9 + cy,
				jk_character_bitmap_5x9_get_bit(bitmap, cy * 5 + cx));
		}
	}
...
for (i = 0 ; i != 26 ; ++i)
{
	jk_display_plot_character(&macos_display, NULL, 'a' + i, i, 0);
	jk_display_plot_character(&macos_display, NULL, 'A' + i, i, 1);
}
