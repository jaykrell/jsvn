...
{
    const unsigned long aa = (b + c + *carry);
    *a = aa;
    *carry = ((aa < b) || (aa < c));
}
...
{
    unsigned long local_low = b->low;
    unsigned long local_high = b->high;
    a->low = -local_low;
    a->high = -local_high - (local_low != 0);
}
...
{
    jk_longlong_t positive_infinity;
    jk_longlong_t negative_infinity;
    jk_longlong_t ll;
    
    
    ll = jk_java_get_long(bytes);
    ...
    )
{
    ...
    i.high =  ((long)(((unsigned long)p[0]) << 24))
            | (((unsigned long)p[1]) << 16)
            | (((unsigned long)p[2]) << 8)
            | ((unsigned long)p[3])
            ;
