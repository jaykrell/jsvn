...
{
   unsigned long  dwcbSize;          // Size of structure
   unsigned long  dwFlags;           // Bit field of IURL_INVOKECOMMAND_FLAGS
   HWND   hwndParent;        // Parent window.  Valid only if IURL_INVOKECOMMAND_FL_ALLOW_UI is set.
   const char* pcszVerb;          // Verb to invoke.  Ignored if IURL_INVOKECOMMAND_FL_USE_DEFAULT_VERB is set.
}
...
typedef struct urlinvokecommandinfoW
{
   ...
   unsigned long   dwFlags;           // Bit field of IURL_INVOKECOMMAND_FLAGS
   HWND    hwndParent;        // Parent window.  Valid only if IURL_INVOKECOMMAND_FL_ALLOW_UI is set.
   const wchar_t* pcszVerb;          // Verb to invoke.  Ignored if IURL_INVOKECOMMAND_FL_USE_DEFAULT_VERB is set.
}
...
INTSHCUTAPI HRESULT __stdcall TranslateURLA(const char* pcszURL,
                                         unsigned long dwInFlags,
                                         char* *ppszTranslatedURL);
INTSHCUTAPI HRESULT __stdcall TranslateURLW(const wchar_t* pcszURL,
                                         unsigned long dwInFlags,
                                         wchar_t* *ppszTranslatedURL);
#ifdef UNICODE
...
INTSHCUTAPI HRESULT __stdcall URLAssociationDialogA(HWND hwndParent,
                                                 unsigned long dwInFlags,
                                                 const char* pcszFile,
                                                 const char* pcszURL,
                                                 char* pszAppBuf,
                                                 UINT ucAppBufLen);
INTSHCUTAPI HRESULT __stdcall URLAssociationDialogW(HWND hwndParent,
                                                 unsigned long dwInFlags,
                                                 const wchar_t* pcszFile,
                                                 const wchar_t* pcszURL,
                                                 wchar_t* pszAppBuf,
                                                 UINT ucAppBufLen);
#ifdef UNICODE
...
INTSHCUTAPI HRESULT __stdcall MIMEAssociationDialogA(HWND hwndParent,
                                                  unsigned long dwInFlags,
                                                  const char* pcszFile,
                                                  const char* pcszMIMEContentType,
                                                  char* pszAppBuf,
                                                  UINT ucAppBufLen);
INTSHCUTAPI HRESULT __stdcall MIMEAssociationDialogW(HWND hwndParent,
                                                  unsigned long dwInFlags,
                                                  const wchar_t* pcszFile,
                                                  const wchar_t* pcszMIMEContentType,
                                                  wchar_t* pszAppBuf,
                                                  UINT ucAppBufLen);
#ifdef UNICODE
