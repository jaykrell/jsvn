...
unsigned long ACMAPI acmGetVersion
(
    void
);
...
typedef BOOL (__stdcall *ACMDRIVERENUMCB)
(
    HACMDRIVERID            hadid,
...
//
typedef LRESULT (__stdcall *ACMDRIVERPROC)(unsigned long, HACMDRIVERID, UINT, LPARAM, LPARAM);
...
{
    unsigned long           cbStruct;           // number of valid bytes in structure
    ...
    unsigned long           vdwACM;             // version of the ACM *compiled* for
    unsigned long           vdwDriver;          // version of the driver

    unsigned long           fdwSupport;         // misc. support flags
    unsigned long           cFormatTags;        // total unique format tags supported
    unsigned long           cFilterTags;        // total unique filter tags supported
...
typedef struct tACMFORMATTAGDETAILSA
{
    ...
    unsigned long           dwFormatTagIndex;
    unsigned long           dwFormatTag;
    unsigned long           cbFormatSize;
    unsigned long           fdwSupport;
    unsigned long           cStandardFormats;
...
#ifdef _WIN32
typedef BOOL (__stdcall *ACMFORMATTAGENUMCBA)
(
    HACMDRIVERID            hadid,
    LPACMFORMATTAGDETAILSA  paftd,
    unsigned long                   dwInstance,
...
typedef BOOL (__stdcall *ACMFORMATTAGENUMCBW)
(
    HACMDRIVERID            hadid,
    LPACMFORMATTAGDETAILSW  paftd,
    unsigned long                   dwInstance,
...
#endif
#else
typedef BOOL (__stdcall *ACMFORMATTAGENUMCB)
(
    HACMDRIVERID            hadid,
    LPACMFORMATTAGDETAILS   paftd,
    unsigned long                   dwInstance,
...
#ifdef _WIN32
typedef BOOL (__stdcall *ACMFORMATENUMCBA)
(
    HACMDRIVERID            hadid,
    LPACMFORMATDETAILSA     pafd,
    unsigned long                   dwInstance,
...
typedef BOOL (__stdcall *ACMFORMATENUMCBW)
(
    HACMDRIVERID            hadid,
    LPACMFORMATDETAILSW     pafd,
    unsigned long                   dwInstance,
...
#endif
#else
typedef BOOL (__stdcall *ACMFORMATENUMCB)
(
    HACMDRIVERID            hadid,
    LPACMFORMATDETAILS      pafd,
    unsigned long                   dwInstance,
...
(
    ...
    unsigned long               cbwfxDst,
    unsigned long               fdwSuggest
);
...
#ifdef _WIN32
typedef UINT (__stdcall *ACMFORMATCHOOSEHOOKPROCA)
(
    HWND                    hwnd,
    UINT                    uMsg,
    WPARAM                  wParam,
...
typedef UINT (__stdcall *ACMFORMATCHOOSEHOOKPROCW)
(
    HWND                    hwnd,
    UINT                    uMsg,
    WPARAM                  wParam,
...
#endif
#else
typedef UINT (__stdcall *ACMFORMATCHOOSEHOOKPROC)
(
    HWND                    hwnd,
    UINT                    uMsg,
    WPARAM                  wParam,
...
{
    unsigned long           cbStruct;           // sizeof(ACMFORMATCHOOSE)
    unsigned long           fdwStyle;           // chooser style flags
    
    HWND            hwndOwner;          // caller's window handle

    LPWAVEFORMATEX  pwfx;               // ptr to wfx buf to receive choice
    unsigned long           cbwfx;              // size of mem buf for pwfx
    const char*          pszTitle;           // dialog box title bar
    
    char            szFormatTag[ACMFORMATTAGDETAILS_FORMATTAG_CHARS];
    char            szFormat[ACMFORMATDETAILS_FORMAT_CHARS];    

    LPSTR           pszName;            // custom name selection
    unsigned long           cchName;            // size in chars of mem buf for pszName

    unsigned long           fdwEnum;            // format enumeration restrictions
    LPWAVEFORMATEX  pwfxEnum;           // format describing restrictions
    
    HINSTANCE       hInstance;          // app instance containing dlg template
    const char*          pszTemplateName;    // custom template name
...
typedef struct tACMFORMATCHOOSEW
{
    ...
    LPWAVEFORMATEX  pwfx;               // ptr to wfx buf to receive choice
    unsigned long           cbwfx;              // size of mem buf for pwfx
    const wchar_t*         pszTitle;           // dialog box title bar
    ...
    HINSTANCE       hInstance;          // app instance containing dlg template
    const wchar_t*         pszTemplateName;    // custom template name
    LPARAM          lCustData;          // data passed to hook fn.
...
{
    unsigned long           cbStruct;
    unsigned long           dwFilterTagIndex;
    unsigned long           dwFilterTag;
    unsigned long           cbFilterSize;
    unsigned long           fdwSupport;
    unsigned long           cStandardFilters;
...
#ifdef _WIN32
typedef BOOL (__stdcall *ACMFILTERTAGENUMCBA)
(
    HACMDRIVERID            hadid,
    LPACMFILTERTAGDETAILSA  paftd,
    unsigned long                   dwInstance,
...
typedef BOOL (__stdcall *ACMFILTERTAGENUMCBW)
(
    HACMDRIVERID            hadid,
    LPACMFILTERTAGDETAILSW  paftd,
    unsigned long                   dwInstance,
...
#endif
#else
typedef BOOL (__stdcall *ACMFILTERTAGENUMCB)
(
    HACMDRIVERID            hadid,
    LPACMFILTERTAGDETAILS   paftd,
    unsigned long                   dwInstance,
...
{
    unsigned long           cbStruct;
    unsigned long           dwFilterIndex;
    unsigned long           dwFilterTag;
    unsigned long           fdwSupport;
    LPWAVEFILTER    pwfltr;
    unsigned long           cbwfltr;
...
#ifdef _WIN32
typedef BOOL (__stdcall *ACMFILTERENUMCBA)
(
    HACMDRIVERID            hadid,
    LPACMFILTERDETAILSA     pafd,
    unsigned long                   dwInstance,
...
typedef BOOL (__stdcall *ACMFILTERENUMCBW)
(
    HACMDRIVERID            hadid,
    LPACMFILTERDETAILSW     pafd,
    unsigned long                   dwInstance,
...
#endif
#else
typedef BOOL (__stdcall *ACMFILTERENUMCB)
(
    HACMDRIVERID            hadid,
    LPACMFILTERDETAILS      pafd,
    unsigned long                   dwInstance,
...
#ifdef _WIN32
typedef UINT (__stdcall *ACMFILTERCHOOSEHOOKPROCA)
(
    HWND                    hwnd,
    UINT                    uMsg,
    WPARAM                  wParam,
...
typedef UINT (__stdcall *ACMFILTERCHOOSEHOOKPROCW)
(
    HWND                    hwnd,
    UINT                    uMsg,
    WPARAM                  wParam,
...
#endif
#else
typedef UINT (__stdcall *ACMFILTERCHOOSEHOOKPROC)
(
    HWND                    hwnd,
    UINT                    uMsg,
    WPARAM                  wParam,
...
{
    unsigned long           cbStruct;           // sizeof(ACMFILTERCHOOSE)
    unsigned long           fdwStyle;           // chooser style flags

    HWND            hwndOwner;          // caller's window handle

    LPWAVEFILTER    pwfltr;             // ptr to wfltr buf to receive choice
    unsigned long           cbwfltr;            // size of mem buf for pwfltr
    ...
    unsigned long           fdwEnum;            // filter enumeration restrictions
...
{
    unsigned long           cbStruct;               // sizeof(ACMSTREAMHEADER)
    unsigned long           fdwStatus;              // ACMSTREAMHEADER_STATUSF_*
    unsigned long           dwUser;                 // user instance data for hdr
    LPBYTE          pbSrc;
    unsigned long           cbSrcLength;
    unsigned long           cbSrcLengthUsed;
    unsigned long           dwSrcUser;              // user instance data for src
    LPBYTE          pbDst;
    unsigned long           cbDstLength;
    unsigned long           cbDstLengthUsed;
    unsigned long           dwDstUser;              // user instance data for dst
    unsigned long           dwReservedDriver[10];   // driver reserved work space
...
(
    ...
    unsigned long                   dwCallback, // callback
    unsigned long                   dwInstance, // callback instance data
    unsigned long                   fdwOpen     // ACM_STREAMOPENF_* and CALLBACK_*
);
