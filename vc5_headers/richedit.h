...
typedef unsigned long (__stdcall *EDITSTREAMCALLBACK)(unsigned long dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb);

typedef struct _editstream
{
	unsigned long dwCookie;		/* user value passed to callback as first parameter */
	unsigned long dwError;		/* last error */
...
struct CHARFORMAT2W : _charformatw
{
	...
	LCID		lcid;				/* Locale ID						*/
	unsigned long		dwReserved;			/* Reserved. Must be 0				*/
...
{
	unsigned long	cb;				/* count of bytes in the string				*/
	unsigned long	flags;			/* flags (see the GT_XXX defines			*/
	UINT	codepage;		/* code page for translation (CP_ACP for default,
						       1200 for Unicode							*/
	const char*	lpDefaultChar;	/* replacement for unmappable chars			*/
...
{
	unsigned long	flags;			/* flags (see GTL_XXX defines)				*/
	UINT	codepage;		/* code page for translation (CP_ACP for default,
							   1200 for Unicode							*/
} GETTEXTLENGTHEX;
