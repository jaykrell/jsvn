...
//#if defined( _WIN32 ) && !defined( _NO_ENUM )
    typedef BOOL ( __stdcall * LPDDENUMCALLBACKA)(GUID  *, char*, char*, void*);
    typedef BOOL ( __stdcall * LPDDENUMCALLBACKW)(GUID  *, wchar_t*, wchar_t*, void*);
    extern HRESULT __stdcall DirectDrawEnumerateW( LPDDENUMCALLBACKW lpCallback, void* lpContext );
    extern HRESULT __stdcall DirectDrawEnumerateA( LPDDENUMCALLBACKA lpCallback, void* lpContext );
    #ifdef UNICODE
    ...
    extern HRESULT __stdcall DirectDrawCreateClipper( unsigned long dwFlags, LPDIRECTDRAWCLIPPER  *lplpDDClipper, IUnknown  *pUnkOuter );
    #ifdef WINNT
	//This is the user-mode entry stub to the kernel mode procedure.
	extern HRESULT NtDirectDrawCreate( GUID  *lpGUID, HANDLE *lplpDD, IUnknown  *pUnkOuter );
    #endif
#endif
...
//#ifndef WINNT
typedef HRESULT ( __stdcall * LPDDENUMMODESCALLBACK)(LPDDSURFACEDESC, void*);
typedef HRESULT ( __stdcall * LPDDENUMSURFACESCALLBACK)(LPDIRECTDRAWSURFACE, LPDDSURFACEDESC, void*);
//#endif
/*
 * DDCOLORKEY
 */
...
typedef struct _DDBLTFX
{
    ...
    unsigned long	dwDDFX;				// FX operations
    unsigned long	dwROP;				// Win32 raster operations
    unsigned long	dwDDROP;			// Raster operations new for DirectDraw
    unsigned long	dwRotationAngle;		// Rotation angle for blt
    unsigned long	dwZBufferOpCode;		// ZBuffer compares
    unsigned long	dwZBufferLow;			// Low limit of Z buffer
    unsigned long	dwZBufferHigh;			// High limit of Z buffer
    unsigned long	dwZBufferBaseDest;		// Destination base value
    unsigned long	dwZDestConstBitDepth;		// Bit depth used to specify Z constant for destination
    union
    {
	unsigned long	dwZDestConst;			// Constant to use as Z buffer for dest
	LPDIRECTDRAWSURFACE lpDDSZBufferDest;	// Surface to use as Z buffer for dest
    };
    unsigned long	dwZSrcConstBitDepth;		// Bit depth used to specify Z constant for source
    union
    {
	unsigned long	dwZSrcConst;			// Constant to use as Z buffer for src
	LPDIRECTDRAWSURFACE lpDDSZBufferSrc;	// Surface to use as Z buffer for src
    };
    unsigned long	dwAlphaEdgeBlendBitDepth;	// Bit depth used to specify constant for alpha edge blend
    unsigned long	dwAlphaEdgeBlend;		// Alpha for edge blending
    unsigned long	dwReserved;
    unsigned long	dwAlphaDestConstBitDepth;	// Bit depth used to specify alpha constant for destination
    union
    {
	unsigned long	dwAlphaDestConst;		// Constant to use as Alpha Channel
	LPDIRECTDRAWSURFACE lpDDSAlphaDest;	// Surface to use as Alpha Channel
    };
    unsigned long	dwAlphaSrcConstBitDepth;	// Bit depth used to specify alpha constant for source
    union
    {
	unsigned long	dwAlphaSrcConst;		// Constant to use as Alpha Channel
	LPDIRECTDRAWSURFACE lpDDSAlphaSrc;	// Surface to use as Alpha Channel
    };
    union
    {
	unsigned long	dwFillColor;			// color in RGB or Palettized
	unsigned long   dwFillDepth;                    // depth value for z-buffer
...
{
    unsigned long	dwSize;			// size of the DDDRIVERCAPS structure
    unsigned long	dwCaps;			// driver specific capabilities
    unsigned long	dwCaps2;		// more driver specific capabilites
    unsigned long	dwCKeyCaps;		// color key capabilities of the surface
    unsigned long	dwFXCaps;		// driver specific stretching and effects capabilites
    unsigned long	dwFXAlphaCaps;		// alpha driver specific capabilities
    unsigned long	dwPalCaps;		// palette capabilities
    unsigned long	dwSVCaps;		// stereo vision capabilities
    unsigned long	dwAlphaBltConstBitDepths;	// DDBD_2,4,8
    unsigned long	dwAlphaBltPixelBitDepths;	// DDBD_1,2,4,8
    unsigned long	dwAlphaBltSurfaceBitDepths;	// DDBD_1,2,4,8
    unsigned long	dwAlphaOverlayConstBitDepths;	// DDBD_2,4,8
    unsigned long	dwAlphaOverlayPixelBitDepths;	// DDBD_1,2,4,8
    unsigned long	dwAlphaOverlaySurfaceBitDepths; // DDBD_1,2,4,8
    unsigned long	dwZBufferBitDepths;		// DDBD_8,16,24,32
    unsigned long	dwVidMemTotal;		// total amount of video memory
    unsigned long	dwVidMemFree;		// amount of free video memory
    unsigned long	dwMaxVisibleOverlays;	// maximum number of visible overlays
    unsigned long	dwCurrVisibleOverlays;	// current number of visible overlays
    unsigned long	dwNumFourCCCodes;	// number of four cc codes
    unsigned long	dwAlignBoundarySrc;	// source rectangle alignment
    unsigned long	dwAlignSizeSrc;		// source rectangle byte size
    unsigned long	dwAlignBoundaryDest;	// dest rectangle alignment
    unsigned long	dwAlignSizeDest;	// dest rectangle byte size
    unsigned long	dwAlignStrideAlign;	// stride alignment
    unsigned long	dwRops[DD_ROP_SPACE];	// ROPS supported
    DDSCAPS	ddsCaps;		// DDSCAPS structure has all the general capabilities
    unsigned long	dwMinOverlayStretch;	// minimum overlay stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
    unsigned long	dwMaxOverlayStretch;	// maximum overlay stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
    unsigned long	dwMinLiveVideoStretch;	// minimum live video stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
    unsigned long	dwMaxLiveVideoStretch;	// maximum live video stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
    unsigned long	dwMinHwCodecStretch;	// minimum hardware codec stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
    unsigned long	dwMaxHwCodecStretch;	// maximum hardware codec stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
    unsigned long	dwReserved1;		// reserved
    unsigned long	dwReserved2;		// reserved
    unsigned long	dwReserved3;		// reserved
    unsigned long	dwSVBCaps;		// driver specific capabilities for System->Vmem blts
    unsigned long	dwSVBCKeyCaps;		// driver color key capabilities for System->Vmem blts
    unsigned long	dwSVBFXCaps;		// driver FX capabilities for System->Vmem blts
    unsigned long	dwSVBRops[DD_ROP_SPACE];// ROPS supported for System->Vmem blts
    unsigned long	dwVSBCaps;		// driver specific capabilities for Vmem->System blts
    unsigned long	dwVSBCKeyCaps;		// driver color key capabilities for Vmem->System blts
    unsigned long	dwVSBFXCaps;		// driver FX capabilities for Vmem->System blts
    unsigned long	dwVSBRops[DD_ROP_SPACE];// ROPS supported for Vmem->System blts
    unsigned long	dwSSBCaps;		// driver specific capabilities for System->System blts
    unsigned long	dwSSBCKeyCaps;		// driver color key capabilities for System->System blts
    unsigned long	dwSSBFXCaps;		// driver FX capabilities for System->System blts
    unsigned long	dwSSBRops[DD_ROP_SPACE];// ROPS supported for System->System blts
    unsigned long	dwReserved4;		// reserved
    unsigned long	dwReserved5;		// reserved
    unsigned long	dwReserved6;		// reserved
} DDCAPS;
...
typedef struct _DDPIXELFORMAT
{
    ...
    union
    {
	unsigned long	dwRGBBitCount;		// how many bits per pixel (BD_4,8,16,24,32)
	unsigned long	dwYUVBitCount;		// how many bits per pixel (BD_4,8,16,24,32)
	unsigned long	dwZBufferBitDepth;	// how many bits for z buffers (BD_8,16,24,32)
	unsigned long	dwAlphaBitDepth;	// how many bits for alpha channels (BD_1,2,4,8)
    };
    union
    {
...
typedef struct _DDOVERLAYFX
{
    unsigned long	dwSize;				// size of structure
    unsigned long	dwAlphaEdgeBlendBitDepth;	// Bit depth used to specify constant for alpha edge blend
    unsigned long	dwAlphaEdgeBlend;		// Constant to use as alpha for edge blend
    unsigned long	dwReserved;
    unsigned long	dwAlphaDestConstBitDepth;	// Bit depth used to specify alpha constant for destination
    union
    {
	unsigned long	dwAlphaDestConst;		// Constant to use as alpha channel for dest
	LPDIRECTDRAWSURFACE lpDDSAlphaDest;	// Surface to use as alpha channel for dest
    };
    unsigned long	dwAlphaSrcConstBitDepth;	// Bit depth used to specify alpha constant for source
    union
    {
	unsigned long	dwAlphaSrcConst;		// Constant to use as alpha channel for src
	LPDIRECTDRAWSURFACE lpDDSAlphaSrc;	// Surface to use as alpha channel for src
    };
    DDCOLORKEY	dckDestColorkey;		// DestColorkey override
    DDCOLORKEY	dckSrcColorkey;			// DestColorkey override
    unsigned long       dwDDFX;                         // Overlay FX
 ...
 * callbacks
 */
typedef unsigned long	( __stdcall *LPCLIPPERCALLBACK)(LPDIRECTDRAWCLIPPER lpDDClipper, HWND hWnd, unsigned long code, void* lpContext );
#ifdef STREAMING
typedef unsigned long	( __stdcall *LPSURFACESTREAMINGCALLBACK)(unsigned long);
#endif
...
DECLARE_INTERFACE_( IDirectDraw, IUnknown )
{
    ...
    STDMETHOD(CreateClipper)(THIS_ unsigned long, LPDIRECTDRAWCLIPPER *, IUnknown  * ) PURE;
    STDMETHOD(CreatePalette)(THIS_ unsigned long, LPPALETTEENTRY, LPDIRECTDRAWPALETTE *, IUnknown  * ) PURE;
    ...
    STDMETHOD(EnumDisplayModes)( THIS_ unsigned long, LPDDSURFACEDESC, void*, LPDDENUMMODESCALLBACK ) PURE;
    STDMETHOD(EnumSurfaces)(THIS_ unsigned long, LPDDSURFACEDESC, void*,LPDDENUMSURFACESCALLBACK ) PURE;
    ...
    STDMETHOD(SetCooperativeLevel)(THIS_ HWND, unsigned long) PURE;
    STDMETHOD(SetDisplayMode)(THIS_ unsigned long, unsigned long,unsigned long) PURE;
    STDMETHOD(WaitForVerticalBlank)(THIS_ unsigned long, HANDLE ) PURE;
};
...
DECLARE_INTERFACE_( IDirectDraw2, IUnknown )
{
    ...
    STDMETHOD(RestoreDisplayMode)(THIS) PURE;
    STDMETHOD(SetCooperativeLevel)(THIS_ HWND, unsigned long) PURE;
    STDMETHOD(SetDisplayMode)(THIS_ unsigned long, unsigned long,unsigned long, unsigned long, unsigned long) PURE;
    STDMETHOD(WaitForVerticalBlank)(THIS_ unsigned long, HANDLE ) PURE;
...
DECLARE_INTERFACE_( IDirectDrawPalette, IUnknown )
{
    ...
    STDMETHOD(GetEntries)(THIS_ unsigned long,unsigned long,unsigned long,LPPALETTEENTRY) PURE;
    STDMETHOD(Initialize)(THIS_ LPDIRECTDRAW, unsigned long, LPPALETTEENTRY) PURE;
    STDMETHOD(SetEntries)(THIS_ unsigned long,unsigned long,unsigned long,LPPALETTEENTRY) PURE;
};
...
DECLARE_INTERFACE_( IDirectDrawClipper, IUnknown )
{
    ...
    STDMETHOD(Initialize)(THIS_ LPDIRECTDRAW, unsigned long) PURE;
    STDMETHOD(IsClipListChanged)(THIS_ BOOL  *) PURE;
    STDMETHOD(SetClipList)(THIS_ LPRGNDATA,unsigned long) PURE;
    STDMETHOD(SetHWnd)(THIS_ unsigned long, HWND ) PURE;
};
...
DECLARE_INTERFACE_( IDirectDrawSurface, IUnknown )
{
    ...
    STDMETHOD(Blt)(THIS_ LPRECT,LPDIRECTDRAWSURFACE, LPRECT,unsigned long, LPDDBLTFX) PURE;
    STDMETHOD(BltBatch)(THIS_ LPDDBLTBATCH, unsigned long, unsigned long ) PURE;
    STDMETHOD(BltFast)(THIS_ unsigned long,unsigned long,LPDIRECTDRAWSURFACE, LPRECT,unsigned long) PURE;
    STDMETHOD(DeleteAttachedSurface)(THIS_ unsigned long,LPDIRECTDRAWSURFACE) PURE;
    STDMETHOD(EnumAttachedSurfaces)(THIS_ void*,LPDDENUMSURFACESCALLBACK) PURE;
    STDMETHOD(EnumOverlayZOrders)(THIS_ unsigned long,void*,LPDDENUMSURFACESCALLBACK) PURE;
    STDMETHOD(Flip)(THIS_ LPDIRECTDRAWSURFACE, unsigned long) PURE;
    STDMETHOD(GetAttachedSurface)(THIS_ LPDDSCAPS, LPDIRECTDRAWSURFACE  *) PURE;
    STDMETHOD(GetBltStatus)(THIS_ unsigned long) PURE;
    STDMETHOD(GetCaps)(THIS_ LPDDSCAPS) PURE;
    STDMETHOD(GetClipper)(THIS_ LPDIRECTDRAWCLIPPER *) PURE;
    STDMETHOD(GetColorKey)(THIS_ unsigned long, LPDDCOLORKEY) PURE;
    STDMETHOD(GetDC)(THIS_ HDC  *) PURE;
    STDMETHOD(GetFlipStatus)(THIS_ unsigned long) PURE;
    ...
    STDMETHOD(Lock)(THIS_ LPRECT,LPDDSURFACEDESC,unsigned long,HANDLE) PURE;
    STDMETHOD(ReleaseDC)(THIS_ HDC) PURE;
    STDMETHOD(Restore)(THIS) PURE;
    STDMETHOD(SetClipper)(THIS_ LPDIRECTDRAWCLIPPER) PURE;
    STDMETHOD(SetColorKey)(THIS_ unsigned long, LPDDCOLORKEY) PURE;
    STDMETHOD(SetOverlayPosition)(THIS_ long, long ) PURE;
    STDMETHOD(SetPalette)(THIS_ LPDIRECTDRAWPALETTE) PURE;
    STDMETHOD(Unlock)(THIS_ void*) PURE;
    STDMETHOD(UpdateOverlay)(THIS_ LPRECT, LPDIRECTDRAWSURFACE,LPRECT,unsigned long, LPDDOVERLAYFX) PURE;
    STDMETHOD(UpdateOverlayDisplay)(THIS_ unsigned long) PURE;
    STDMETHOD(UpdateOverlayZOrder)(THIS_ unsigned long, LPDIRECTDRAWSURFACE) PURE;
};
...
DECLARE_INTERFACE_( IDirectDrawSurface2, IUnknown )
{
    ...
    STDMETHOD(AddOverlayDirtyRect)(THIS_ LPRECT) PURE;
    STDMETHOD(Blt)(THIS_ LPRECT,LPDIRECTDRAWSURFACE2, LPRECT,unsigned long, LPDDBLTFX) PURE;
    STDMETHOD(BltBatch)(THIS_ LPDDBLTBATCH, unsigned long, unsigned long ) PURE;
    STDMETHOD(BltFast)(THIS_ unsigned long,unsigned long,LPDIRECTDRAWSURFACE2, LPRECT,unsigned long) PURE;
    STDMETHOD(DeleteAttachedSurface)(THIS_ unsigned long,LPDIRECTDRAWSURFACE2) PURE;
    STDMETHOD(EnumAttachedSurfaces)(THIS_ void*,LPDDENUMSURFACESCALLBACK) PURE;
    STDMETHOD(EnumOverlayZOrders)(THIS_ unsigned long,void*,LPDDENUMSURFACESCALLBACK) PURE;
    STDMETHOD(Flip)(THIS_ LPDIRECTDRAWSURFACE2, unsigned long) PURE;
    ...
    STDMETHOD(SetPalette)(THIS_ LPDIRECTDRAWPALETTE) PURE;
    STDMETHOD(Unlock)(THIS_ void*) PURE;
    STDMETHOD(UpdateOverlay)(THIS_ LPRECT, LPDIRECTDRAWSURFACE2,LPRECT,unsigned long, LPDDOVERLAYFX) PURE;
    STDMETHOD(UpdateOverlayDisplay)(THIS_ unsigned long) PURE;
    STDMETHOD(UpdateOverlayZOrder)(THIS_ unsigned long, LPDIRECTDRAWSURFACE2) PURE;
    /*** Added in the v2 interface ***/
    STDMETHOD(GetDDInterface)(THIS_ void*  *) PURE;
    STDMETHOD(PageLock)(THIS_ unsigned long) PURE;
    STDMETHOD(PageUnlock)(THIS_ unsigned long) PURE;
};
...
typedef struct _DDSURFACEDESC
{
	...
	unsigned long		dwZBufferBitDepth;	// depth of Z buffer requested
