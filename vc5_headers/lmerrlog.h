...
typedef struct _ERROR_LOG {
     unsigned long         el_len;
     unsigned long         el_reserved;
     unsigned long         el_time;
     unsigned long         el_error;
     LPWSTR        el_name;             // pointer to service name
     LPWSTR        el_text;             // pointer to string array
     LPBYTE        el_data;             // pointer to BYTE array
     unsigned long         el_data_size;        // byte count of el_data area
     unsigned long         el_nstrings;         // number of strings in el_text.
...
NetErrorLogClear (
    IN const wchar_t* server,
    IN const wchar_t* backupfile,
    IN LPBYTE  reserved
    );
...
#define NELOG_Ioctl_Error       (ERRLOG_BASE + 8)
    ...
    *     unsigned long  approx CS:IP of call to ioctl or fsctl
    *     WORD   error code
...
#define NELOG_Wksta_Infoseg     (ERRLOG_BASE + 60)
    ...
    *  The size follows, in unsigned long format:
    */
