...
void* WINAPI WOWGetVDMPointer(unsigned long vp, unsigned long dwBytes,
                               BOOL fProtectedMode);
...
void* WINAPI WOWGetVDMPointerFix(unsigned long vp, unsigned long dwBytes,
                                  BOOL fProtectedMode);
VOID WINAPI WOWGetVDMPointerUnfix(unsigned long vp);
...
WORD  WINAPI WOWGlobalAlloc16(WORD wFlags, unsigned long cb);
WORD  WINAPI WOWGlobalFree16(WORD hMem);
unsigned long WINAPI WOWGlobalLock16(WORD hMem);
...
unsigned long WINAPI WOWGlobalAllocLock16(WORD wFlags, unsigned long cb, WORD *phMem);
WORD  WINAPI WOWGlobalUnlockFree16(unsigned long vpMem);
unsigned long WINAPI WOWGlobalLockSize16(WORD hMem, PDWORD pcb);
...
#define HTASK_32(h16)	  ((unsigned long)     (WOWHandle32(h16, WOW_TYPE_HTASK)))
...
// returns TRUE and the unsigned long pointed to by pdwRetCode contains
...
//
// LONG  PASCAL SetWindowText(HWND hwnd, const char* lpsz);
//
...
// first for DWORDs and offset first for  pointers.
//

unsigned long WINAPI WOWCallback16(unsigned long vpfn16, unsigned long dwParam);
...
BOOL WINAPI WOWCallback16Ex(
                unsigned long  vpfn16,
                unsigned long  dwFlags,
                unsigned long  cbArgs,
                void*  pArgs,
                PDWORD pdwRetCode
                );
