...
struct VolLockBroadcast {
	...
	unsigned long	vlb_owner;		// thread on which lock request is being issued
 ...
 * STARTING WITH A unsigned long SIZE AND HAVING NO POINTER IN THE STRUCT.
 *
 */
...
struct _DEV_BROADCAST_HEADER { /* */
    unsigned long	dbcd_size;
    unsigned long	dbcd_devicetype;
    unsigned long       dbcd_reserved;
};

struct _DEV_BROADCAST_OEM {	/* */
    unsigned long	dbco_size;
    unsigned long	dbco_devicetype;
    unsigned long       dbco_reserved;
    unsigned long	dbco_identifier;
    unsigned long	dbco_suppfunc;
};
...
struct _DEV_BROADCAST_NET { /* */
    unsigned long	dbcn_size;
    unsigned long	dbcn_devicetype;
    unsigned long       dbcn_reserved;
    unsigned long	dbcn_resource;
    unsigned long	dbcn_flags;
};
