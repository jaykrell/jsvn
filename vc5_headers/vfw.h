...
#else
#define VFWAPI   PASCAL
#define VFWAPIV  CDECL
#endif
...
unsigned long  PASCAL VideoForWindowsVersion(void);
...
#define MKFOURCC( ch0, ch1, ch2, ch3 )                                    \
		( (unsigned long)(BYTE)(ch0) | ( (unsigned long)(BYTE)(ch1) << 8 ) |	\
		( (unsigned long)(BYTE)(ch2) << 16 ) | ( (unsigned long)(BYTE)(ch3) << 24 ) )
#endif
...
typedef struct {
    unsigned long               dwSize;         // sizeof(ICOPEN)
    unsigned long               fccType;        // 'vidc'
    unsigned long               fccHandler;     //
    unsigned long               dwVersion;      // version of compman opening you
    unsigned long               dwFlags;        // LOWORD is type specific
    LRESULT             dwError;        // error return.
    void*              pV1Reserved;    // Reserved
    void*              pV2Reserved;    // Reserved
    unsigned long               dnDevNode;      // Devnode for PnP devices
} ICOPEN;
...
typedef struct {
    unsigned long   dwSize;                 // sizeof(ICINFO)
    unsigned long   fccType;                // compressor type     'vidc' 'audc'
    unsigned long   fccHandler;             // compressor sub-type 'rle ' 'jpeg' 'pcm '
    unsigned long   dwFlags;                // flags LOWORD is type specific
    unsigned long   dwVersion;              // version of the driver
    unsigned long   dwVersionICM;           // version of the ICM used
    //
...
typedef struct {
    ...
    unsigned long		dwRate;		// frame rate, as always
    unsigned long		dwScale;

    unsigned long		dwOverheadPerFrame;
    unsigned long		dwReserved2;

    LONG (__stdcall *GetData)(LPARAM lInput, LONG lFrame, void* lpBits, LONG len);
    LONG (__stdcall *PutData)(LPARAM lOutput, LONG lFrame, void* lpBits, LONG len);
} ICCOMPRESSFRAMES;
...
typedef struct {
    ...
    LONG (__stdcall *Status) (LPARAM lParam, UINT message, LONG l);
} ICSETSTATUSPROC;
...
typedef struct {
    unsigned long               dwFlags;    // flags (from AVI index...)
    ...
    void*              lpOutput;
    unsigned long		ckid;	    // ckid from AVI file
} ICDECOMPRESS;
...
typedef struct {
    ...
    void*              lpSrc;      // compressed data

    LPBITMAPINFOHEADER  lpbiDst;    // DIB to decompress to
    void*              lpDst;      // output data
...
typedef struct {
    ...
    unsigned long               dwRate;         // frames/second = (dwRate/dwScale)
    unsigned long               dwScale;
...
typedef struct {
    unsigned long               dwFlags;        // flags
    void*		lpFormat;       // format of frame to decompress
    void*              lpData;         // frame data to decompress
...
BOOL    VFWAPI ICInfo(unsigned long fccType, unsigned long fccHandler, ICINFO  * lpicinfo);
BOOL    VFWAPI ICInstall(unsigned long fccType, unsigned long fccHandler, LPARAM lParam, LPSTR szDesc, UINT wFlags);
BOOL    VFWAPI ICRemove(unsigned long fccType, unsigned long fccHandler, UINT wFlags);
LRESULT VFWAPI ICGetInfo(HIC hic, ICINFO  *picinfo, unsigned long cb);

HIC     VFWAPI ICOpen(unsigned long fccType, unsigned long fccHandler, UINT wMode);
HIC     VFWAPI ICOpenFunction(unsigned long fccType, unsigned long fccHandler, UINT wMode, FARPROC lpfnHandler);
LRESULT VFWAPI ICClose(HIC hic);

LRESULT VFWAPI  ICSendMessage(HIC hic, UINT msg, unsigned long dw1, unsigned long dw2);
#ifndef _WIN32
...
#define ICQueryAbout(hic) \
    (ICSendMessage(hic, ICM_ABOUT, (unsigned long) -1, ICMF_ABOUT_QUERY) == ICERR_OK)

#define ICAbout(hic, hwnd) \
    ICSendMessage(hic, ICM_ABOUT, (unsigned long)(UINT)(hwnd), 0)

#define ICQueryConfigure(hic) \
    (ICSendMessage(hic, ICM_CONFIGURE, (unsigned long) -1, ICMF_CONFIGURE_QUERY) == ICERR_OK)

#define ICConfigure(hic, hwnd) \
    ICSendMessage(hic, ICM_CONFIGURE, (unsigned long)(UINT)(hwnd), 0)
...
#define ICGetState(hic, pv, cb) \
    ICSendMessage(hic, ICM_GETSTATE, (unsigned long)(void*)(pv), (unsigned long)(cb))

#define ICSetState(hic, pv, cb) \
    ICSendMessage(hic, ICM_SETSTATE, (unsigned long)(void*)(pv), (unsigned long)(cb))
...
************************************************************************/
static unsigned long dwICValue;

#define ICGetDefaultQuality(hic) \
    (ICSendMessage(hic, ICM_GETDEFAULTQUALITY, (unsigned long)(void*)&dwICValue, sizeof(unsigned long)), dwICValue)

#define ICGetDefaultKeyFrameRate(hic) \
    (ICSendMessage(hic, ICM_GETDEFAULTKEYFRAMERATE, (unsigned long)(void*)&dwICValue, sizeof(unsigned long)), dwICValue)
...
#define ICDrawWindow(hic, prc) \
    ICSendMessage(hic, ICM_DRAW_WINDOW, (unsigned long)(void*)(prc), sizeof(RECT))
 ...
 *
 */
unsigned long VFWAPIV ICCompress(
    HIC                 hic,
    unsigned long               dwFlags,        // flags
    LPBITMAPINFOHEADER  lpbiOutput,     // output format
    void*              lpData,         // output data
    LPBITMAPINFOHEADER  lpbiInput,      // format of frame to compress
    void*              lpBits,         // frame data to compress
    LPDWORD             lpckid,         // ckid for data in AVI file
    LPDWORD             lpdwFlags,      // flags in the AVI index.
    LONG                lFrameNum,      // frame number of seq.
    unsigned long               dwFrameSize,    // reqested size in bytes. (if non zero)
    unsigned long               dwQuality,      // quality within one frame
    LPBITMAPINFOHEADER  lpbiPrev,       // format of previous frame
    void*              lpPrev);        // previous frame
...
#define ICCompressBegin(hic, lpbiInput, lpbiOutput) \
    ICSendMessage(hic, ICM_COMPRESS_BEGIN, (unsigned long)(void*)(lpbiInput), (unsigned long)(void*)(lpbiOutput))
...
#define ICCompressQuery(hic, lpbiInput, lpbiOutput) \
    ICSendMessage(hic, ICM_COMPRESS_QUERY, (unsigned long)(void*)(lpbiInput), (unsigned long)(void*)(lpbiOutput))
...
#define ICCompressGetFormat(hic, lpbiInput, lpbiOutput) \
    ICSendMessage(hic, ICM_COMPRESS_GET_FORMAT, (unsigned long)(void*)(lpbiInput), (unsigned long)(void*)(lpbiOutput))
...
#define ICCompressGetSize(hic, lpbiInput, lpbiOutput) \
    ICSendMessage(hic, ICM_COMPRESS_GET_SIZE, (unsigned long)(void*)(lpbiInput), (unsigned long)(void*)(lpbiOutput))
...
unsigned long VFWAPIV ICDecompress(
    HIC                 hic,
    unsigned long               dwFlags,    // flags (from AVI index...)
...
#define ICDecompressBegin(hic, lpbiInput, lpbiOutput) \
    ICSendMessage(hic, ICM_DECOMPRESS_BEGIN, (unsigned long)(void*)(lpbiInput), (unsigned long)(void*)(lpbiOutput))
...
#define ICDecompressQuery(hic, lpbiInput, lpbiOutput) \
    ICSendMessage(hic, ICM_DECOMPRESS_QUERY, (unsigned long)(void*)(lpbiInput), (unsigned long)(void*)(lpbiOutput))
...
#define ICDecompressGetFormat(hic, lpbiInput, lpbiOutput) \
    ((LONG) ICSendMessage(hic, ICM_DECOMPRESS_GET_FORMAT, (unsigned long)(void*)(lpbiInput), (unsigned long)(void*)(lpbiOutput)))
...
#define ICDecompressGetPalette(hic, lpbiInput, lpbiOutput) \
    ICSendMessage(hic, ICM_DECOMPRESS_GET_PALETTE, (unsigned long)(void*)(lpbiInput), (unsigned long)(void*)(lpbiOutput))

#define ICDecompressSetPalette(hic, lpbiPalette) \
    ICSendMessage(hic, ICM_DECOMPRESS_SET_PALETTE, (unsigned long)(void*)(lpbiPalette), 0)
            ...
            int dyDst)
{
    ...
    return ICSendMessage(hic, ICM_DECOMPRESSEX, (unsigned long)&ic, sizeof(ic));
}
            ...
            int dyDst)
{
    ...
    // note that ICM swaps round the length and pointer
    // length in lparam2, pointer in lparam1
    return ICSendMessage(hic, ICM_DECOMPRESSEX_BEGIN, (unsigned long)&ic, sizeof(ic));
}
            ...
            int dyDst)
{
    ...
    // note that ICM swaps round the length and pointer
    // length in lparam2, pointer in lparam1
    return ICSendMessage(hic, ICM_DECOMPRESSEX_QUERY, (unsigned long)&ic, sizeof(ic));
}
...
unsigned long VFWAPIV ICDrawBegin(
        HIC                 hic,
        unsigned long               dwFlags,        // flags
        HPALETTE            hpal,           // palette to draw with
        HWND                hwnd,           // window to draw to
            ...
            HIC hicDecomp)
{
    ...
    // note that ICM swaps round the length and pointer
    // length in lparam2, pointer in lparam1
    return ICSendMessage(hic, ICM_DRAW_SUGGESTFORMAT, (unsigned long)&ic, sizeof(ic));
}
...
#define ICDrawQuery(hic, lpbiInput) \
    ICSendMessage(hic, ICM_DRAW_QUERY, (unsigned long)(void*)(lpbiInput), 0L)

#define ICDrawChangePalette(hic, lpbiInput) \
    ICSendMessage(hic, ICM_DRAW_CHANGEPALETTE, (unsigned long)(void*)(lpbiInput), 0L)

#define ICGetBuffersWanted(hic, lpdwBuffers) \
    ICSendMessage(hic, ICM_GETBUFFERSWANTED, (unsigned long)(void*)(lpdwBuffers), 0)
...
#define ICDrawStartPlay(hic, lFrom, lTo) \
    ICSendMessage(hic, ICM_DRAW_START_PLAY, (unsigned long)(lFrom), (unsigned long)(lTo))
...
#define ICDrawGetTime(hic, lplTime) \
    ICSendMessage(hic, ICM_DRAW_GETTIME, (unsigned long)(void*)(lplTime), 0)

#define ICDrawSetTime(hic, lTime) \
    ICSendMessage(hic, ICM_DRAW_SETTIME, (unsigned long)lTime, 0)

#define ICDrawRealize(hic, hdc, fBackground) \
    ICSendMessage(hic, ICM_DRAW_REALIZE, (unsigned long)(UINT)(HDC)(hdc), (unsigned long)(BOOL)(fBackground))
...
ICSetStatusProc(
            ...
            unsigned long dwFlags,
            LRESULT lParam,
            LONG (__stdcall *fpfnStatus)(LPARAM, UINT, LONG) )
{
    ICSETSTATUSPROC ic;
    ...
    // note that ICM swaps round the length and pointer
    // length in lparam2, pointer in lparam1
    return ICSendMessage(hic, ICM_SET_STATUS_PROC, (unsigned long)&ic, sizeof(ic));
}
...
HIC  VFWAPI ICLocate(unsigned long fccType, unsigned long fccHandler, LPBITMAPINFOHEADER lpbiIn, LPBITMAPINFOHEADER lpbiOut, WORD wFlags);
...
HANDLE VFWAPI ICImageDecompress(
        ...
        void*              lpBits,     // data to decompress
...
typedef struct {
    ...
    unsigned long		dwFlags;	// see below...
    HIC			hic;		// HIC of chosen compressor
    unsigned long               fccType;	// basically ICTYPE_VIDEO
    unsigned long               fccHandler;	// handler of chosen compressor or
					// "" or "DIB "
...
BOOL VFWAPI ICCompressorChoose(
        ...
        UINT        uiFlags,            // flags
        void*      pvIn,               // input format (optional)
        void*      lpData,             // input data (optional)
...
void* VFWAPI ICSeqCompressFrame(
    PCOMPVARS               pc,         // set by ICCompressorChoose
    UINT                    uiFlags,    // flags
    void*                  lpBits,     // input DIB bits
...
**
*/
extern void* VFWAPI DrawDibGetBuffer(HDRAWDIB hdd, LPBITMAPINFOHEADER lpbi, unsigned long dwSize, unsigned long dwFlags);
...
**
*/
extern BOOL VFWAPI DrawDibStart(HDRAWDIB hdd, unsigned long rate);
...
unsigned long VFWAPI DrawDibProfileDisplay(LPBITMAPINFOHEADER lpbi);
...
typedef struct {
    unsigned long	fccType;		    /* stream type, for consistency */
    unsigned long       fccHandler;                 /* compressor */
    unsigned long       dwKeyFrameEvery;            /* keyframe rate */
    unsigned long       dwQuality;                  /* compress quality 0-10,000 */
    unsigned long       dwBytesPerSecond;           /* bytes per second */
    unsigned long       dwFlags;                    /* flags... see below */
    void*      lpFormat;                   /* save format */
    unsigned long       cbFormat;
    void*      lpParms;                    /* compressor options */
    unsigned long       cbParms;
    unsigned long       dwInterleaveEvery;          /* for non-video streams only */
...
DECLARE_INTERFACE_(IAVIStream, IUnknown)
{
    ...
    STDMETHOD(ReadFormat)  (THIS_ LONG lPos,
			    void* lpFormat, LONG  *lpcbFormat) PURE ;
    STDMETHOD(SetFormat)   (THIS_ LONG lPos,
			    void* lpFormat, LONG cbFormat) PURE ;
    STDMETHOD(Read)        (THIS_ LONG lStart, LONG lSamples,
			    void* lpBuffer, LONG cbBuffer,
			    LONG  * plBytes, LONG  * plSamples) PURE ;
    STDMETHOD(Write)       (THIS_ LONG lStart, LONG lSamples,
			    void* lpBuffer, LONG cbBuffer,
			    unsigned long dwFlags,
			    LONG  *plSampWritten,
			    LONG  *plBytesWritten) PURE ;
    STDMETHOD(Delete)      (THIS_ LONG lStart, LONG lSamples) PURE;
    STDMETHOD(ReadData)    (THIS_ unsigned long fcc, void* lp, LONG  *lpcb) PURE ;
    STDMETHOD(WriteData)   (THIS_ unsigned long fcc, void* lp, LONG cb) PURE ;
#ifdef _WIN32
    STDMETHOD(SetInfo) (THIS_ AVISTREAMINFOW  * lpInfo,
			    LONG cbInfo) PURE;
#else
...
DECLARE_INTERFACE_(IGetFrame, IUnknown)
{
    ...
    STDMETHOD_(void*,GetFrame) (THIS_ LONG lPos) PURE;
//  STDMETHOD_(void*,GetFrameData) (THIS_ LONG lPos) PURE;

    STDMETHOD(Begin) (THIS_ LONG lStart, LONG lEnd, LONG lRate) PURE;
    STDMETHOD(End) (THIS) PURE;

    STDMETHOD(SetFormat) (THIS_ LPBITMAPINFOHEADER lpbi, void* lpBits, int x, int y, int dx, int dy) PURE;
...
STDAPI_(unsigned long) AVIFileAddRef       (PAVIFILE pfile);
STDAPI_(unsigned long) AVIFileRelease      (PAVIFILE pfile);

#ifdef _WIN32
STDAPI AVIFileOpenA       (PAVIFILE  * ppfile, const char* szFile,
			  UINT uMode, LPCLSID lpHandler);
STDAPI AVIFileOpenW       (PAVIFILE  * ppfile, const wchar_t* szFile,
			  UINT uMode, LPCLSID lpHandler);
#ifdef UNICODE
...
#endif
#else // win16
STDAPI AVIFileOpen       (PAVIFILE  * ppfile, const char* szFile,
			  UINT uMode, LPCLSID lpHandler);
#endif
...
STDAPI AVIFileGetStream     (PAVIFILE pfile, PAVISTREAM  * ppavi, unsigned long fccType, LONG lParam);
...
STDAPI_(unsigned long) AVIStreamAddRef       (PAVISTREAM pavi);
STDAPI_(unsigned long) AVIStreamRelease      (PAVISTREAM pavi);
...
STDAPI AVIStreamReadFormat   (PAVISTREAM pavi, LONG lPos,void* lpFormat,LONG  *lpcbFormat);
STDAPI AVIStreamSetFormat    (PAVISTREAM pavi, LONG lPos,void* lpFormat,LONG cbFormat);
STDAPI AVIStreamReadData     (PAVISTREAM pavi, unsigned long fcc, void* lp, LONG  *lpcb);
STDAPI AVIStreamWriteData    (PAVISTREAM pavi, unsigned long fcc, void* lp, LONG cb);
...
STDAPI AVIStreamWrite        (PAVISTREAM pavi,
			      ...
			      void* lpBuffer, LONG cbBuffer, unsigned long dwFlags,
			      LONG  *plSampWritten,
			      LONG  *plBytesWritten);
...
STDAPI_(void*) AVIStreamGetFrame(PGETFRAME pg, LONG lPos);
...
// Shortcut function
STDAPI AVIStreamOpenFromFileA(PAVISTREAM  *ppavi, const char* szFile,
			     unsigned long fccType, LONG lParam,
			     UINT mode, CLSID  *pclsidHandler);
STDAPI AVIStreamOpenFromFileW(PAVISTREAM  *ppavi, const wchar_t* szFile,
			     unsigned long fccType, LONG lParam,
			     UINT mode, CLSID  *pclsidHandler);
#ifdef UNICODE
...
STDAPI AVIStreamCreate(PAVISTREAM  *ppavi, LONG lParam1, LONG lParam2,
		       CLSID  *pclsidHandler);
...
EXTERN_C HRESULT CDECL AVISaveA (const char*               szFile,
		CLSID  *pclsidHandler,
...
STDAPI AVISaveVA(const char*               szFile,
		CLSID  *pclsidHandler,
		AVISAVECALLBACK     lpfnCallback,
		int                 nStreams,
		PAVISTREAM  *    ppavi,
		LPAVICOMPRESSOPTIONS  *plpOptions);
EXTERN_C HRESULT CDECL AVISaveW (const wchar_t*               szFile,
		CLSID  *pclsidHandler,
		AVISAVECALLBACK     lpfnCallback,
		int                 nStreams,
		PAVISTREAM	    pfile,
...
STDAPI AVISaveVW(const wchar_t*               szFile,
		CLSID  *pclsidHandler,
		AVISAVECALLBACK     lpfnCallback,
		int                 nStreams,
		PAVISTREAM  *    ppavi,
...
#endif
STDAPI AVIMakeFileFromStreams(PAVIFILE  *	ppfile,
			       int		nStreams,
...
STDAPI EditStreamSetNameA(PAVISTREAM pavi, const char* lpszName);
STDAPI EditStreamSetNameW(PAVISTREAM pavi, const wchar_t* lpszName);
...
HWND VFWAPIV MCIWndCreateA(HWND hwndParent, HINSTANCE hInstance,
		      unsigned long dwStyle,const char* szFile);
HWND VFWAPIV MCIWndCreateW(HWND hwndParent, HINSTANCE hInstance,
		      unsigned long dwStyle,const wchar_t* szFile);
#ifdef UNICODE
...
#define MCIWndSave(hwnd, szFile)    (LONG)MCIWndSM(hwnd, MCI_SAVE, 0, (LPARAM)(void*)(szFile))
#define MCIWndSaveDialog(hwnd)      MCIWndSave(hwnd, -1)

// if you dont give a device it will use the current device....
#define MCIWndNew(hwnd, lp)         (LONG)MCIWndSM(hwnd, MCIWNDM_NEW, 0, (LPARAM)(void*)(lp))

#define MCIWndRecord(hwnd)          (LONG)MCIWndSM(hwnd, MCI_RECORD, 0, 0)
#define MCIWndOpen(hwnd, sz, f)     (LONG)MCIWndSM(hwnd, MCIWNDM_OPEN, (WPARAM)(UINT)(f),(LPARAM)(void*)(sz))
...
#define MCIWndReturnString(hwnd, lp, len)  (LONG)MCIWndSM(hwnd, MCIWNDM_RETURNSTRING, (WPARAM)(UINT)(len), (LPARAM)(void*)(lp))
#define MCIWndGetError(hwnd, lp, len) (LONG)MCIWndSM(hwnd, MCIWNDM_GETERROR, (WPARAM)(UINT)(len), (LPARAM)(void*)(lp))
...
#define MCIWndGetFileName(hwnd, lp, len) (LONG)MCIWndSM(hwnd, MCIWNDM_GETFILENAME, (WPARAM)(UINT)(len), (LPARAM)(void*)(lp))
#define MCIWndGetDevice(hwnd, lp, len)   (LONG)MCIWndSM(hwnd, MCIWNDM_GETDEVICE, (WPARAM)(UINT)(len), (LPARAM)(void*)(lp))
...
typedef struct videohdr_tag {
    ...
    unsigned long       dwBytesUsed;            /* Bytes actually used */
    unsigned long       dwTimeCaptured;         /* Milliseconds from start of stream */
    unsigned long       dwUser;                 /* for client's use */
    unsigned long       dwFlags;                /* assorted flags (see defines) */
    unsigned long       dwReserved[4];          /* reserved for driver */
...
typedef struct channel_caps_tag {
    unsigned long       dwFlags;                /* Capability flags*/
    unsigned long       dwSrcRectXMod;          /* Granularity of src rect in x */
    unsigned long       dwSrcRectYMod;          /* Granularity of src rect in y */
    unsigned long       dwSrcRectWidthMod;      /* Granularity of src rect width */
    unsigned long       dwSrcRectHeightMod;     /* Granularity of src rect height */
    unsigned long       dwDstRectXMod;          /* Granularity of dst rect in x */
    unsigned long       dwDstRectYMod;          /* Granularity of dst rect in y */
    unsigned long       dwDstRectWidthMod;      /* Granularity of dst rect width */
    unsigned long       dwDstRectHeightMod;     /* Granularity of dst rect height */
...
#define capSetCallbackOnError(hwnd, fpProc)        ((BOOL)AVICapSM(hwnd, WM_CAP_SET_CALLBACK_ERROR, 0, (LPARAM)(void*)(fpProc)))
#define capSetCallbackOnStatus(hwnd, fpProc)       ((BOOL)AVICapSM(hwnd, WM_CAP_SET_CALLBACK_STATUS, 0, (LPARAM)(void*)(fpProc)))
#define capSetCallbackOnYield(hwnd, fpProc)        ((BOOL)AVICapSM(hwnd, WM_CAP_SET_CALLBACK_YIELD, 0, (LPARAM)(void*)(fpProc)))
#define capSetCallbackOnFrame(hwnd, fpProc)        ((BOOL)AVICapSM(hwnd, WM_CAP_SET_CALLBACK_FRAME, 0, (LPARAM)(void*)(fpProc)))
#define capSetCallbackOnVideoStream(hwnd, fpProc)  ((BOOL)AVICapSM(hwnd, WM_CAP_SET_CALLBACK_VIDEOSTREAM, 0, (LPARAM)(void*)(fpProc)))
#define capSetCallbackOnWaveStream(hwnd, fpProc)   ((BOOL)AVICapSM(hwnd, WM_CAP_SET_CALLBACK_WAVESTREAM, 0, (LPARAM)(void*)(fpProc)))
#define capSetCallbackOnCapControl(hwnd, fpProc)   ((BOOL)AVICapSM(hwnd, WM_CAP_SET_CALLBACK_CAPCONTROL, 0, (LPARAM)(void*)(fpProc)))
...
#define capDriverGetName(hwnd, szName, wSize)      ((BOOL)AVICapSM(hwnd, WM_CAP_DRIVER_GET_NAME, (WPARAM)(wSize), (LPARAM)(void*)(LPTSTR)(szName)))
#define capDriverGetVersion(hwnd, szVer, wSize)    ((BOOL)AVICapSM(hwnd, WM_CAP_DRIVER_GET_VERSION, (WPARAM)(wSize), (LPARAM)(void*)(LPTSTR)(szVer)))
#define capDriverGetCaps(hwnd, s, wSize)           ((BOOL)AVICapSM(hwnd, WM_CAP_DRIVER_GET_CAPS, (WPARAM)(wSize), (LPARAM)(void*)(LPCAPDRIVERCAPS)(s)))

#define capFileSetCaptureFile(hwnd, szName)        ((BOOL)AVICapSM(hwnd, WM_CAP_FILE_SET_CAPTURE_FILE, 0, (LPARAM)(void*)(LPTSTR)(szName)))
#define capFileGetCaptureFile(hwnd, szName, wSize) ((BOOL)AVICapSM(hwnd, WM_CAP_FILE_GET_CAPTURE_FILE, (WPARAM)(wSize), (LPARAM)(void*)(LPTSTR)(szName)))
#define capFileAlloc(hwnd, dwSize)                 ((BOOL)AVICapSM(hwnd, WM_CAP_FILE_ALLOCATE, 0, (LPARAM)(unsigned long)(dwSize)))
#define capFileSaveAs(hwnd, szName)                ((BOOL)AVICapSM(hwnd, WM_CAP_FILE_SAVEAS, 0, (LPARAM)(void*)(LPTSTR)(szName)))
...
#define capFileSaveDIB(hwnd, szName)               ((BOOL)AVICapSM(hwnd, WM_CAP_FILE_SAVEDIB, 0, (LPARAM)(void*)(LPTSTR)(szName)))

#define capEditCopy(hwnd)                          ((BOOL)AVICapSM(hwnd, WM_CAP_EDIT_COPY, 0, 0L))

#define capSetAudioFormat(hwnd, s, wSize)          ((BOOL)AVICapSM(hwnd, WM_CAP_SET_AUDIOFORMAT, (WPARAM)(wSize), (LPARAM)(void*)(LPWAVEFORMATEX)(s)))
#define capGetAudioFormat(hwnd, s, wSize)          ((unsigned long)AVICapSM(hwnd, WM_CAP_GET_AUDIOFORMAT, (WPARAM)(wSize), (LPARAM)(void*)(LPWAVEFORMATEX)(s)))
#define capGetAudioFormatSize(hwnd)                ((unsigned long)AVICapSM(hwnd, WM_CAP_GET_AUDIOFORMAT, (WPARAM)0, (LPARAM)0L))
...
#define capGetVideoFormat(hwnd, s, wSize)          ((unsigned long)AVICapSM(hwnd, WM_CAP_GET_VIDEOFORMAT, (WPARAM)(wSize), (LPARAM)(void*)(s)))
#define capGetVideoFormatSize(hwnd)            ((unsigned long)AVICapSM(hwnd, WM_CAP_GET_VIDEOFORMAT, 0, 0L))
#define capSetVideoFormat(hwnd, s, wSize)          ((BOOL)AVICapSM(hwnd, WM_CAP_SET_VIDEOFORMAT, (WPARAM)(wSize), (LPARAM)(void*)(s)))
...
#define capGetStatus(hwnd, s, wSize)               ((BOOL)AVICapSM(hwnd, WM_CAP_GET_STATUS, (WPARAM)(wSize), (LPARAM)(void*)(LPCAPSTATUS)(s)))
...
#define capCaptureGetSetup(hwnd, s, wSize)         ((BOOL)AVICapSM(hwnd, WM_CAP_GET_SEQUENCE_SETUP, (WPARAM)(wSize), (LPARAM)(void*)(LPCAPTUREPARMS)(s)))
#define capCaptureSetSetup(hwnd, s, wSize)         ((BOOL)AVICapSM(hwnd, WM_CAP_SET_SEQUENCE_SETUP, (WPARAM)(wSize), (LPARAM)(void*)(LPCAPTUREPARMS)(s)))

#define capSetMCIDeviceName(hwnd, szName)          ((BOOL)AVICapSM(hwnd, WM_CAP_SET_MCI_DEVICE, 0, (LPARAM)(void*)(LPTSTR)(szName)))
#define capGetMCIDeviceName(hwnd, szName, wSize)   ((BOOL)AVICapSM(hwnd, WM_CAP_GET_MCI_DEVICE, (WPARAM)(wSize), (LPARAM)(void*)(LPTSTR)(szName)))

#define capPaletteOpen(hwnd, szName)               ((BOOL)AVICapSM(hwnd, WM_CAP_PAL_OPEN, 0, (LPARAM)(void*)(LPTSTR)(szName)))
#define capPaletteSave(hwnd, szName)               ((BOOL)AVICapSM(hwnd, WM_CAP_PAL_SAVE, 0, (LPARAM)(void*)(LPTSTR)(szName)))
#define capPalettePaste(hwnd)                      ((BOOL)AVICapSM(hwnd, WM_CAP_PAL_PASTE, (WPARAM) 0, (LPARAM)0L))
#define capPaletteAuto(hwnd, iFrames, iColors)     ((BOOL)AVICapSM(hwnd, WM_CAP_PAL_AUTOCREATE, (WPARAM)(iFrames), (LPARAM)(unsigned long)(iColors)))
#define capPaletteManual(hwnd, fGrab, iColors)     ((BOOL)AVICapSM(hwnd, WM_CAP_PAL_MANUALCREATE, (WPARAM)(fGrab), (LPARAM)(unsigned long)(iColors)))
...
typedef struct tagCapStatus {
    ...
    unsigned long       dwCurrentVideoFrame;        // # of video frames cap'td
    unsigned long       dwCurrentVideoFramesDropped;// # of video frames dropped
    unsigned long       dwCurrentWaveSamples;       // # of wave samples cap'td
    unsigned long       dwCurrentTimeElapsedMS;     // Elapsed capture duration
    HPALETTE    hPalCurrent;                // Current palette in use
    BOOL        fCapturingNow;              // Capture in progress?
    unsigned long       dwReturn;                   // Error value after any operation
...
typedef struct tagCaptureParms {
    unsigned long       dwRequestMicroSecPerFrame;  // Requested capture rate
    ...
    unsigned long       dwIndexSize;                // Max index size in frames (32K)
    ...
    unsigned long       dwMCIStartTime;             // Time to start in MS
    unsigned long       dwMCIStopTime;              // Time to stop in MS
    BOOL        fStepCaptureAt2x;           // Perform spatial averaging 2x
    UINT        wStepCaptureAverageFrames;  // Temporal average n Frames
    unsigned long       dwAudioBufferSize;          // Size of audio bufs (0 = default)
...
typedef LRESULT (__stdcall* CAPSTATUSCALLBACKW) (HWND hWnd, int nID, const wchar_t* lpsz);
typedef LRESULT (__stdcall* CAPERRORCALLBACKW)  (HWND hWnd, int nID, const wchar_t* lpsz);
typedef LRESULT (__stdcall* CAPSTATUSCALLBACKA) (HWND hWnd, int nID, const char* lpsz);
typedef LRESULT (__stdcall* CAPERRORCALLBACKA)  (HWND hWnd, int nID, const char* lpsz);
#ifdef UNICODE
