...
typedef unsigned long DPID,  *LPDPID;

typedef struct _DPCAPS
{
    unsigned long   dwSize;
    unsigned long   dwFlags;
    unsigned long   dwMaxBufferSize;
    unsigned long   dwMaxQueueSize;             // Function of DPlay, not SP.
    unsigned long   dwMaxPlayers;
    unsigned long   dwHundredBaud;              // 24 is 2400, 96 is 9600, etc.
    unsigned long   dwLatency;
} DPCAPS;
...
typedef struct
{
    ...
    unsigned long   dwSession;                    // session identifier
    unsigned long   dwMaxPlayers;                 // Maximum players allowed in game.
    unsigned long   dwCurrentPlayers;             // Current  players in Game.
    unsigned long   dwFlags;                      // DPOPEN_* flags
    char    szSessionName[DPSESSIONNAMELEN];// Human readable name for Game
    char    szUserField[DPUSERRESERVED];
    unsigned long   dwReserved1;                  // Reserved for future MS use.
    char    szPassword[DPPASSWORDLEN];    // Password to be allowed into game.
    unsigned long   dwReserved2;                  // Reserved for future MS use. 
 ...
 * Create API
 */
typedef BOOL ( __stdcall * LPDPENUMDPCALLBACK)(
    LPGUID              lpSPGuid,
    char*               lpFriendlyName,
...
typedef BOOL ( __stdcall * LPDPENUMSESSIONSCALLBACK)(
    LPDPSESSIONDESC     lpDPSGameDesc,
    void*              lpContext,
    unsigned long*             lpdwTimeOut,
    unsigned long               dwFlags);



HRESULT __stdcall DirectPlayCreate( LPGUID lpGUID, LPDIRECTPLAY  *lplpDP, IUnknown  *pUnk);
HRESULT __stdcall DirectPlayEnumerate( LPDPENUMDPCALLBACK, void* );


/* Player enumeration callback prototype */
typedef BOOL ( __stdcall *LPDPENUMPLAYERSCALLBACK)(
    DPID    dpId,
    char*   lpFriendlyName,
    char*   lpFormalName,
    unsigned long   dwFlags,
...
DECLARE_INTERFACE_( IDirectPlay, IUnknown )
{
    ...
    STDMETHOD(EnumGroupPlayers)     (THIS_ DPID, LPDPENUMPLAYERSCALLBACK,void*,unsigned long) PURE;
    STDMETHOD(EnumGroups)           (THIS_ unsigned long, LPDPENUMPLAYERSCALLBACK,void*,unsigned long) PURE;
    STDMETHOD(EnumPlayers)          (THIS_ unsigned long, LPDPENUMPLAYERSCALLBACK,void*,unsigned long) PURE;
    STDMETHOD(EnumSessions)         (THIS_ LPDPSESSIONDESC,unsigned long,LPDPENUMSESSIONSCALLBACK,void*,unsigned long) PURE;
    STDMETHOD(GetCaps)              (THIS_ LPDPCAPS) PURE;
    STDMETHOD(GetMessageCount)      (THIS_ DPID, unsigned long*) PURE;
    STDMETHOD(GetPlayerCaps)        (THIS_ DPID, LPDPCAPS) PURE;
    STDMETHOD(GetPlayerName)        (THIS_ DPID,char*,unsigned long*,char*,unsigned long*) PURE;
    STDMETHOD(Initialize)           (THIS_ LPGUID) PURE;
    STDMETHOD(Open)                 (THIS_ LPDPSESSIONDESC) PURE;
    STDMETHOD(Receive)              (THIS_ LPDPID,LPDPID,unsigned long,void*,unsigned long*) PURE;
    STDMETHOD(SaveSession)          (THIS_ char*) PURE;
    STDMETHOD(Send)                 (THIS_ DPID, DPID, unsigned long, void*, unsigned long) PURE;
