...
typedef struct wavefilter_tag {
    unsigned long   cbStruct;           /* Size of the filter in bytes */
    unsigned long   dwFilterTag;        /* filter type */
    unsigned long   fdwFilter;          /* Flags for the filter (Universal Dfns) */
    unsigned long   dwReserved[5];      /* Reserved for system use */
} WAVEFILTER;
...
typedef struct tagJPEGINFOHEADER {
    ...
    unsigned long       JPEGProcess;

    /* Process specific fields */
    unsigned long       JPEGColorSpaceID;
    unsigned long       JPEGBitsPerSample;
    unsigned long       JPEGHSubSampling;
    unsigned long       JPEGVSubSampling;
} JPEGINFOHEADER;
