...
// a single 32-bit unsigned long, 2 16-bit USHORTs or 4 8-bit BYTEs
//
...
typedef struct _LLC_CCB {
    ...
    unsigned long ulCompletionFlag;         // used in command completion
...
typedef struct {
    ...
    void* pBuffer;                  // any buffer in memory
    unsigned long cbBufferSize;             // buffer size in bytes
    unsigned long cbMinimumSizeThreshold;   // minimum locked size
...
typedef struct {
    ...
    UCHAR cGroupCount;              // number of group SAPs of this SAP
    PUCHAR pGroupList;              // offset to the group list
    unsigned long DlcStatusFlags;           // User notify flag for DLC status changes
...
typedef struct {
    ...
    UCHAR uchAccessPriority;        // token ring access priority
    void* pRemoteNodeAddress;       // pointer to the destination address
...
typedef struct {
    ...
    void* AlertEvent;               // alerting event
} LLC_DLC_SET_THRESHOLD_PARMS, *PLLC_DLC_SET_THRESHOLD_PARMS;

typedef struct {
    void* TraceBuffer;              // trace buffer
    unsigned long TraceBufferSize;          // trace buffer size
    unsigned long TraceFlags;               // various trace flags
...
typedef struct {
    unsigned long cTransmittedFrames;
    unsigned long cReceivedFrames;
    unsigned long cDiscardedFrames;
    unsigned long cDataLost;
...
typedef struct {
    ...
    unsigned long ulProtocolTypeMask;
    unsigned long ulProtocolTypeMatch;
...
typedef struct {
    unsigned long ulAdapterCheckFlag;
    unsigned long ulNetworkStatusFlag;
    unsigned long ulPcErrorFlag;
    unsigned long ulSystemActionFlag;
...
typedef struct {
    ...
    unsigned long ulMaxFrameLength;         // maximum frame length (only in Windows/Nt)
...
typedef struct {
    ...
    unsigned long ulNotificationFlag;
...
typedef struct {
    ...
    unsigned long ulReceiveFlag;            // the received data handler
...
typedef struct {
    ...
    void* pBuffer1;                 // yet another segment
    void* pBuffer2;                 // this is the last segment of frame
...
typedef struct {
    ...
    UCHAR Reserved2[2];
    unsigned long cXmitBufferCount;
