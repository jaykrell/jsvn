...
STDAPI_(BOOL) OleUIAddVerbMenuW(LPOLEOBJECT lpOleObj, const wchar_t* lpszShortType,
        HMENU hMenu, UINT uPos, UINT uIDVerbMin, UINT uIDVerbMax,
        BOOL bAddConvert, UINT idConvert, HMENU *lphMenu);
STDAPI_(BOOL) OleUIAddVerbMenuA(LPOLEOBJECT lpOleObj, const char* lpszShortType,
        HMENU hMenu, UINT uPos, UINT uIDVerbMin, UINT uIDVerbMax,
        BOOL bAddConvert, UINT idConvert, HMENU *lphMenu);
#ifdef UNICODE
...
typedef struct tagOLEUIINSERTOBJECTW
{
        ...
        const wchar_t*         lpszCaption;    // Dialog caption bar contents
        LPFNOLEUIHOOK   lpfnHook;       // Hook callback
        LPARAM          lCustData;      // Custom data to pass to hook
        HINSTANCE       hInstance;      // Instance for customized template name
        const wchar_t*         lpszTemplate;   // Customized template name
        ...
        unsigned long           oleRender;      // IN: Rendering option
        ...
        void*          *ppvObj;        // OUT: Where the object is returned.
...
typedef struct tagOLEUIINSERTOBJECTA
{
        ...
        unsigned long           dwFlags;        // IN-OUT:  Flags
        HWND            hWndOwner;      // Owning window
        const char*          lpszCaption;    // Dialog caption bar contents
        LPFNOLEUIHOOK   lpfnHook;       // Hook callback
        LPARAM          lCustData;      // Custom data to pass to hook
        HINSTANCE       hInstance;      // Instance for customized template name
        const char*          lpszTemplate;   // Customized template name
        HRSRC           hResource;      // Customized template handle
...
typedef struct tagOLEUIPASTEENTRYW
{
   ...
   const wchar_t*          lpstrFormatName;// String that represents the format
                                                                        // to the user. %s is replaced by the
                                                                        // full user type name of the object.
   const wchar_t*          lpstrResultText;// String to customize the result text
   ...
   unsigned long            dwScratchSpace; // Scratch space used internally.

} OLEUIPASTEENTRYW, *POLEUIPASTEENTRYW, *LPOLEUIPASTEENTRYW;
typedef struct tagOLEUIPASTEENTRYA
{
   FORMATETC        fmtetc;         // Format that is acceptable.
   const char*           lpstrFormatName;// String that represents the format
                                                                        // to the user. %s is replaced by the
                                                                        // full user type name of the object.
   const char*           lpstrResultText;// String to customize the result text
                                                                        // of the dialog when the user
                                                                        // selects the format correspoding to
                                                                        // this entry. Any %s in this string
                                                                        // is replaced by the the application
...
typedef struct tagOLEUIPASTESPECIALW
{
        ...
        UINT *       arrLinkTypes;   // IN: List of acceptable link types
...
DECLARE_INTERFACE_(IOleUILinkContainerW, IUnknown)
{
        ...
        STDMETHOD(SetLinkUpdateOptions) (THIS_ unsigned long dwLink,
                unsigned long dwUpdateOpt) PURE;
        STDMETHOD(GetLinkUpdateOptions) (THIS_ unsigned long dwLink,
                unsigned long * lpdwUpdateOpt) PURE;
        STDMETHOD(SetLinkSource) (THIS_ unsigned long dwLink, LPWSTR lpszDisplayName,
                unsigned long lenFileName, unsigned long * pchEaten, BOOL fValidateSource) PURE;
        STDMETHOD(GetLinkSource) (THIS_ unsigned long dwLink,
                LPWSTR * lplpszDisplayName, unsigned long * lplenFileName,
        ...
        STDMETHOD(UpdateLink) (THIS_ unsigned long dwLink,
...
DECLARE_INTERFACE_(IOleUILinkContainerA, IUnknown)
{
        ...
        STDMETHOD(GetLinkUpdateOptions) (THIS_ unsigned long dwLink,
                unsigned long * lpdwUpdateOpt) PURE;
        STDMETHOD(SetLinkSource) (THIS_ unsigned long dwLink, LPSTR lpszDisplayName,
                unsigned long lenFileName, unsigned long * pchEaten, BOOL fValidateSource) PURE;
        STDMETHOD(GetLinkSource) (THIS_ unsigned long dwLink,
                LPSTR * lplpszDisplayName, unsigned long * lplenFileName,
...
typedef struct tagOLEUIBUSYW
{
        // These IN fields are standard across all OLEUI dialog functions.
        unsigned long           cbStruct;       // Structure Size
        unsigned long           dwFlags;        // IN-OUT: see below
        HWND            hWndOwner;      // Owning window
        const wchar_t*         lpszCaption;    // Dialog caption bar contents
        LPFNOLEUIHOOK   lpfnHook;       // Hook callback
        LPARAM          lCustData;      // Custom data to pass to hook
...
typedef struct tagOLEUICHANGESOURCEW
{
        ...
        unsigned long           dwReserved1[4]; // (reserved for future use)

        // Specifics for OLEUICHANGESOURCE.
        LPOLEUILINKCONTAINERW lpOleUILinkContainer;  // IN: used to validate link sources
        unsigned long           dwLink;         // IN: magic# for lpOleUILinkContainer
        LPWSTR          lpszDisplayName;// IN-OUT: complete source display name
        unsigned long           nFileLength;    // IN-OUT: file moniker part of lpszDisplayName
...
DECLARE_INTERFACE_(IOleUIObjInfoW, IUnknown)
{
        ...
        STDMETHOD(GetObjectInfo) (THIS_ unsigned long dwObject,
                unsigned long * lpdwObjSize, LPWSTR * lplpszLabel,
                LPWSTR * lplpszType, LPWSTR * lplpszShortType,
                LPWSTR * lplpszLocation) PURE;
        STDMETHOD(GetConvertInfo) (THIS_ unsigned long dwObject,
                CLSID * lpClassID, WORD * lpwFormat,
                CLSID * lpConvertDefaultClassID,
                LPCLSID * lplpClsidExclude, UINT * lpcClsidExclude) PURE;
        STDMETHOD(ConvertObject) (THIS_ unsigned long dwObject, REFCLSID clsidNew) PURE;

        // *** extra for View Properties *** //
        STDMETHOD(GetViewInfo) (THIS_ unsigned long dwObject,
                HGLOBAL * phMetaPict, unsigned long* pdvAspect, int* pnCurrentScale) PURE;
        STDMETHOD(SetViewInfo) (THIS_ unsigned long dwObject,
                HGLOBAL hMetaPict, unsigned long dvAspect,
...
DECLARE_INTERFACE_(IOleUIObjInfoA, IUnknown)
{
        ...
        // *** extra for General Properties *** //
        STDMETHOD(GetObjectInfo) (THIS_ unsigned long dwObject,
                unsigned long * lpdwObjSize, LPSTR * lplpszLabel,
                LPSTR * lplpszType, LPSTR * lplpszShortType,
                LPSTR * lplpszLocation) PURE;
        STDMETHOD(GetConvertInfo) (THIS_ unsigned long dwObject,
                CLSID * lpClassID, WORD * lpwFormat,
                CLSID * lpConvertDefaultClassID,
                LPCLSID * lplpClsidExclude, UINT * lpcClsidExclude) PURE;
...
DECLARE_INTERFACE_(IOleUILinkInfoW, IOleUILinkContainerW)
{
        ...
        STDMETHOD(GetLastUpdate) (THIS_ unsigned long dwLink,
                FILETIME * lpLastUpdate) PURE;
};
...
typedef struct tagOLEUIGNRLPROPSW
{
        ...
        unsigned long           cbStruct;       // Structure Size
        unsigned long           dwFlags;        // IN-OUT: flags specific to general page
        unsigned long           dwReserved1[2];
        LPFNOLEUIHOOK   lpfnHook;       // Hook callback
        LPARAM          lCustData;      // Custom data to pass to hook
...
typedef struct tagOLEUIVIEWPROPSW
{
        // These IN fields are standard across all OLEUI property pages.
        unsigned long           cbStruct;       // Structure Size
        unsigned long           dwFlags;        // IN-OUT: flags specific to view page
        unsigned long           dwReserved1[2];
...
typedef struct tagOLEUILINKPROPSW
{
        // These IN fields are standard across all OLEUI property pages.
        unsigned long           cbStruct;       // Structure Size
        unsigned long           dwFlags;        // IN-OUT: flags specific to links page
        unsigned long           dwReserved1[2];
        LPFNOLEUIHOOK   lpfnHook;       // Hook callback (not used in this dialog)
        LPARAM          lCustData;      // Custom data to pass to hook
        unsigned long           dwReserved2[3];
...
typedef struct tagOLEUIOBJECTPROPSW
{
        ...
        unsigned long           cbStruct;       // Structure Size
        unsigned long           dwFlags;        // IN-OUT: global flags for the sheet
