 ...
 */
typedef void (__stdcall MSGCALLRELEASE)(
    unsigned long       ulCallerData, 
    LPMESSAGE   lpMessage );
...
STDAPI_(SCODE) OpenIMsgSession(
    ...
    LPMSGSESS    *lppMsgSess );      /* <- message session object    */
...
STDAPI_(SCODE) OpenIMsgOnIStg( 
    ...
    LPMALLOC        lpMalloc,           /* -> Co malloc object               */
    void*          lpMapiSup,          /* -> MAPI Support Obj (optional)    */
    LPSTORAGE       lpStg,              /* -> open IStorage containing msg   */
    MSGCALLRELEASE  *lpfMsgCallRelease,  /* -> release callback rtn (opt) */
    unsigned long           ulCallerData,       /* caller data returned in callback  */
    unsigned long           ulFlags,            /* -> flags (controls istg commit)   */
    LPMESSAGE        *lppMsg );      /* <- open message object            */

#define IMSG_NO_ISTG_COMMIT     ((unsigned long) 0x00000001)
...
#define PROPATTR_MANDATORY      ((unsigned long) 0x00000001)
#define PROPATTR_READABLE       ((unsigned long) 0x00000002)
#define PROPATTR_WRITEABLE      ((unsigned long) 0x00000004)

#define PROPATTR_NOT_PRESENT    ((unsigned long) 0x00000008)
