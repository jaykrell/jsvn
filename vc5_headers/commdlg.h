...
#endif // !UNICODE
short __stdcall     GetFileTitleA(const char*, LPSTR, WORD);
short __stdcall     GetFileTitleW(const wchar_t*, LPWSTR, WORD);
#ifdef UNICODE
...
#define CommDlg_OpenSave_GetFolderIDList(_hdlg, _pidl, _cbmax) \
        (int)SNDMSG(_hdlg, CDM_GETFOLDERIDLIST, (WPARAM)_cbmax, (LPARAM)(void*)_pidl)
...
typedef struct tagFINDREPLACEA {
   ...
   const char*       lpTemplateName;     // custom template name
...
typedef struct tagFINDREPLACEW {
   ...
   LPARAM       lCustData;          // data passed to hook fn.
   LPFRHOOKPROC lpfnHook;           // ptr. to hook fn. or NULL
   const wchar_t*      lpTemplateName;     // custom template name
...
typedef struct tagCHOOSEFONTA {
   ...
   unsigned long           Flags;              // enum. type flags
...
unsigned long __stdcall     CommDlgExtendedError(void);
