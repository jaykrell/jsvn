...
HRESULT __stdcall NdrDllRegisterProxy(
    IN HMODULE                  hDll,
    IN const ProxyFileInfo **   pProxyFileList,
    IN const CLSID *            pclsid);

HRESULT __stdcall NdrDllUnregisterProxy(
    IN HMODULE                  hDll,
    IN const ProxyFileInfo **   pProxyFileList,
    IN const CLSID *            pclsid);
...
#define REGISTER_PROXY_DLL_ROUTINES(pProxyFileList, pClsID) \
    ...
    HRESULT __stdcall DLLREGISTERSERVER_ENTRY() \
    { \
    ...
    HRESULT __stdcall DLLUNREGISTERSERVER_ENTRY() \
    { \
...
#define DLLGETCLASSOBJECTROUTINE(pPFlist, pClsid,pFactory)    \
 HRESULT __stdcall DLLGETCLASSOBJECT_ENTRY ( \
     REFCLSID rclsid, \
     REFIID riid, \
     void ** ppv ) \
        { \
...
#define DLLCANUNLOADNOW(pFactory)   \
 HRESULT __stdcall DLLCANUNLOADNOW_ENTRY()    \
    {   \
...
unsigned long __stdcall CStdStubBuffer_Release(IRpcStubBuffer *This) \
    {   \
...
unsigned long __stdcall CStdStubBuffer2_Release(IRpcStubBuffer *This) \
    {   \
