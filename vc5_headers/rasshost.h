...
typedef struct _SECURITY_MESSAGE
{
    ...
    unsigned long dwError;                  // Should be non-zero only if error
...
typedef struct _RAS_SECURITY_INFO
{
    ...
    unsigned long LastError;                    // SUCCESS = receive completed
                                        // PENDING = receive pending
                                        // else completed with error

    unsigned long BytesReceived;                // only valid if LastError == SUCCESS
...
typedef unsigned long (WINAPI *RASSECURITYPROC)();
...
RasSecurityDialogBegin(
    ...
    IN unsigned long  SendBufSize,  // Size of above bufer in bytes
    IN PBYTE  pRecvBuf,     // Pointer to the buffer used in
                            // RasSecurityDialogReceive
    IN unsigned long  RecvBufSize,  // Size of above buffer
...
RasSecurityDialogReceive(
    ...
    IN unsigned long    Timeout,        // in seconds
