...
typedef struct _FMS_GETDRIVEINFOA {      // for drive
   unsigned long dwTotalSpace;
   unsigned long dwFreeSpace;
...
typedef struct _FMS_LOADA {
   unsigned long dwSize;                        // for version checks
...
typedef unsigned long (__stdcall *FM_EXT_PROC)(HWND, WORD, LONG);
typedef unsigned long (__stdcall *FM_UNDELETE_PROC)(HWND, LPTSTR);
