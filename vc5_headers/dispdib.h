...
// function prototypes
unsigned  __stdcall DisplayDib(LPBITMAPINFOHEADER lpbi, char* lpBits, WORD wFlags);
unsigned  __stdcall DisplayDibEx(LPBITMAPINFOHEADER lpbi, int x, int y, char* lpBits, WORD wFlags);
...
//
__inline unsigned DisplayDibWindowMessage(HWND hwnd, unsigned msg, WPARAM wParam, LPARAM lParam, unsigned long cbSize)
{
#if defined(_WIN32) || defined(WIN32)
        COPYDATASTRUCT cds;
        cds.dwData = MAKELONG(msg, wParam);
        cds.cbData = lParam ? cbSize : 0;
        cds.lpData = (void*)lParam;
        return (unsigned)SendMessage(hwnd, WM_COPYDATA, (WPARAM)(HWND)NULL, (LPARAM)(void*)&cds);
#else
        return (unsigned)SendMessage(hwnd, msg, wParam, lParam);
#endif
}
...
//
__inline HWND DisplayDibWindowCreateEx(HWND hwndParent, HINSTANCE hInstance, unsigned long dwStyle)
{
#if defined(_WIN32) || defined(WIN32)
    unsigned long show = 2;
    unsigned long zero = 0;
    void* params[4] = {NULL, &zero, &show, 0};

    if ((unsigned)LoadModule(DISPLAYDIB_DLL, &params) < (unsigned)HINSTANCE_ERROR)
        return NULL;    // loading DISPDIB did not work
#else
    if ((unsigned)LoadLibrary(DISPLAYDIB_DLL) < (unsigned)HINSTANCE_ERROR)
        return NULL;    // loading DISPDIB did not work
#endif
...
#define DisplayDibWindowSetFmt(hwnd, lpbi)              DisplayDibWindowMessage(hwnd, DDM_SETFMT, 0, (LPARAM)(void*)(lpbi), sizeof(BITMAPINFOHEADER) + 256 * sizeof(RGBQUAD))
#define DisplayDibWindowDraw(hwnd, flags, bits, size)   DisplayDibWindowMessage(hwnd, DDM_DRAW, (WPARAM)(unsigned)(flags), (LPARAM)(void*)(bits), (unsigned long)(size))

#ifdef __cplusplus
#define DisplayDibWindowBegin(hwnd)                     ::ShowWindow(hwnd, SW_SHOWNORMAL)
#define DisplayDibWindowEnd(hwnd)                       ::ShowWindow(hwnd, SW_HIDE)
#define DisplayDibWindowBeginEx(hwnd, f)                ::SendMessage(hwnd, DDM_BEGIN, (WPARAM)(unsigned)(f), 0)
