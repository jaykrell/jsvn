...
*/
typedef long (__stdcall* CONFNOTIFYPROC)(HCONF, unsigned long, unsigned long, void*, void*, unsigned long);
...
typedef struct _CONFADDRW {
	unsigned long dwSize;                // size of this structure, in bytes
	unsigned long dwAddrType;            // type of address that follows
	union {
		unsigned long dwIp;              // IP Address (a.b.c.d)
		const wchar_t* psz;             // pointer to a null terminated string
		};
...
typedef struct _CONFADDRA {
	...
	union {
		unsigned long dwIp;              // IP Address (a.b.c.d)
		const char* psz;              // pointer to a null terminated string
		};
...
typedef struct tagConfDest {
	unsigned long dwSize;                   // size of this structure, in bytes
	unsigned long dwFlags;                  // destination flags (CONF_DF_xxx)
	unsigned long dwUserId;                 // unique user identifier
...
typedef struct _CONFINFOW {
	...
	unsigned long cUsers;
	unsigned long dwGCCID;
...
typedef struct _CONFGUIDW {
	unsigned long   dwSize;
	GUID    guid;
	const wchar_t* pszApplication;
	const wchar_t* pszCommandLine;
...
typedef struct _CONFFILEINFOW {
	...
	unsigned long dwBytesTransferred;
...
#define CONFN_DATA_SENT              0x0101 // unsigned long,    LPCONFDEST, void*
#define CONFN_DATA_RECEIVED          0x0102 // unsigned long,    LPCONFDEST, void*
...
#ifndef ANSI_ONLY
unsigned long __stdcall ConferenceConnectW(HCONF * phConf, LPCONFADDRW lpConfAddr, LPCONFINFO lpConfInfo, LPCONFNOTIFY lpConfNotify);
unsigned long __stdcall ConferenceSendFileW(HCONF hConf, LPCONFDEST lpConfDest, const wchar_t* szFileName, unsigned long dwFlags);
unsigned long __stdcall ConferenceGetInfoW(HCONF hConf, unsigned long dwCode, void* lpv);
unsigned long __stdcall ConferenceSetInfoW(HCONF hConf, unsigned long dwCode, void* lpv);
#endif //!ANSI_ONLY
#ifndef UNICODE_ONLY
unsigned long __stdcall ConferenceConnectA(HCONF * phConf, LPCONFADDRA lpConfAddr, LPCONFINFO lpConfInfo, LPCONFNOTIFY lpConfNotify);
unsigned long __stdcall ConferenceSendFileA(HCONF hConf, LPCONFDEST lpConfDest, const char* szFileName, unsigned long dwFlags);
unsigned long __stdcall ConferenceGetInfoA(HCONF hConf, unsigned long dwCode, void* lpv);
unsigned long __stdcall ConferenceSetInfoA(HCONF hConf, unsigned long dwCode, void* lpv);
#endif //!UNICODE_ONLY
#ifdef UNICODE
...
unsigned long __stdcall ConferenceListen(unsigned long dwReserved);
unsigned long __stdcall ConferenceDisconnect(HCONF hConf);
unsigned long __stdcall ConferenceSetNotify(HCONF hConf, LPCONFNOTIFY lpConfNotify, HCONFNOTIFY * phConfNotify);
unsigned long __stdcall ConferenceRemoveNotify(HCONF hConf, HCONFNOTIFY hConfNotify);
unsigned long __stdcall ConferenceCancelTransfer(HCONF hConf, unsigned long dwFileId);
unsigned long __stdcall ConferenceSendData(HCONF hConf, LPCONFDEST lpConfDest, void* lpv, unsigned long cb, unsigned long dwFlags);
unsigned long __stdcall ConferenceLaunchRemote(HCONF hConf, LPCONFDEST lpConfDest, unsigned long dwReserved);
unsigned long __stdcall ConferenceShareWindow(HCONF hConf, HWND hwnd, unsigned long dwCode);
