...
{
	unsigned long			cbStruct;			// Size of structure
	...
	unsigned long			dvaspect;			// Display aspect to use
	unsigned long			dwFlags;			// Object status flags
	unsigned long			dwUser;				// Dword for user's use
} REOBJECT;
...
#define REO_CP_SELECTION ((unsigned long) -1L)

// Use character position to specify object instead of index
#define REO_IOB_SELECTION ((unsigned long) -1L)
#define REO_IOB_USE_CP ((unsigned long) -2L)
...
{
    // *** IUnknown methods ***
    STDMETHOD(QueryInterface) (THIS_ REFIID riid, void*  * lplpObj) PURE;
	...
	STDMETHOD(GetObject) (THIS_ LONG iob, REOBJECT  * lpreobject,
						  unsigned long dwFlags) PURE;
    STDMETHOD(InsertObject) (THIS_ REOBJECT  * lpreobject) PURE;
	STDMETHOD(ConvertObject) (THIS_ LONG iob, REFCLSID rclsidNew,
							  const char* lpstrUserTypeNew) PURE;
	STDMETHOD(ActivateAs) (THIS_ REFCLSID rclsid, REFCLSID rclsidAs) PURE;
	STDMETHOD(SetHostNames) (THIS_ const char* lpstrContainerApp,
							 const char* lpstrContainerObj) PURE;
	STDMETHOD(SetLinkAvailable) (THIS_ LONG iob, BOOL fAvailable) PURE;
	STDMETHOD(SetDvaspect) (THIS_ LONG iob, unsigned long dvaspect) PURE;
	...
	STDMETHOD(GetClipboardData) (THIS_ CHARRANGE  * lpchrg, unsigned long reco,
...
DECLARE_INTERFACE_(IRichEditOleCallback, IUnknown)
{
	...
	STDMETHOD(QueryAcceptData) (THIS_ LPDATAOBJECT lpdataobj,
								CLIPFORMAT  * lpcfFormat, unsigned long reco,
								BOOL fReally, HGLOBAL hMetaPict) PURE;
	STDMETHOD(ContextSensitiveHelp) (THIS_ BOOL fEnterMode) PURE;
	STDMETHOD(GetClipboardData) (THIS_ CHARRANGE  * lpchrg, unsigned long reco,
									LPDATAOBJECT  * lplpdataobj) PURE;
	STDMETHOD(GetDragDropEffect) (THIS_ BOOL fDrag, unsigned long grfKeyState,
