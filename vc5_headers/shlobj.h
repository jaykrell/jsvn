...
typedef struct _CMINVOKECOMMANDINFO {
    unsigned long cbSize;        // sizeof(CMINVOKECOMMANDINFO)
    unsigned long fMask;         // any combination of CMIC_MASK_*
    HWND hwnd;           // might be NULL (indicating no owner window)
    const char* lpVerb;       // either a string or MAKEINTRESOURCE(idOffset)
    const char* lpParameters; // might be NULL (indicating no parameter)
    const char* lpDirectory;  // might be NULL (indicating no specific directory)
...
typedef struct _CMInvokeCommandInfoEx {
    unsigned long cbSize;        // must be sizeof(CMINVOKECOMMANDINFOEX)
    unsigned long fMask;         // any combination of CMIC_MASK_*
    HWND hwnd;           // might be NULL (indicating no owner window)
    const char* lpVerb;       // either a string or MAKEINTRESOURCE(idOffset)
    const char* lpParameters; // might be NULL (indicating no parameter)
    ...
    unsigned long dwHotKey;
    HANDLE hIcon;
    const char* lpTitle;      // For CreateProcess-StartupInfo.lpTitle
    const wchar_t* lpVerbW;        // Unicode verb (for those who can use it)
    const wchar_t* lpParametersW;  // Unicode parameters (for those who can use it)
    const wchar_t* lpDirectoryW;   // Unicode directory (for those who can use it)
    const wchar_t* lpTitleW;       // Unicode title (for those who can use it)
...
DECLARE_INTERFACE_(IShellLinkA, IUnknown)       // sl
{
    ...
    STDMETHOD(GetPath)(THIS_ LPSTR pszFile, int cchMaxPath, WIN32_FIND_DATAA *pfd, unsigned long fFlags) PURE;
    ...
    STDMETHOD(SetDescription)(THIS_ const char* pszName) PURE;

    STDMETHOD(GetWorkingDirectory)(THIS_ LPSTR pszDir, int cchMaxPath) PURE;
    STDMETHOD(SetWorkingDirectory)(THIS_ const char* pszDir) PURE;

    STDMETHOD(GetArguments)(THIS_ LPSTR pszArgs, int cchMaxPath) PURE;
    STDMETHOD(SetArguments)(THIS_ const char* pszArgs) PURE;
    ...
    STDMETHOD(SetIconLocation)(THIS_ const char* pszIconPath, int iIcon) PURE;

    STDMETHOD(SetRelativePath)(THIS_ const char* pszPathRel, unsigned long dwReserved) PURE;

    STDMETHOD(Resolve)(THIS_ HWND hwnd, unsigned long fFlags) PURE;

    STDMETHOD(SetPath)(THIS_ const char* pszFile) PURE;
};
...
DECLARE_INTERFACE_(IShellLinkW, IUnknown)       // sl
{
    ...
    STDMETHOD(GetPath)(THIS_ LPWSTR pszFile, int cchMaxPath, WIN32_FIND_DATAW *pfd, unsigned long fFlags) PURE;

    STDMETHOD(GetIDList)(THIS_ LPITEMIDLIST * ppidl) PURE;
    STDMETHOD(SetIDList)(THIS_ LPCITEMIDLIST pidl) PURE;

    STDMETHOD(GetDescription)(THIS_ LPWSTR pszName, int cchMaxName) PURE;
    STDMETHOD(SetDescription)(THIS_ const wchar_t* pszName) PURE;

    STDMETHOD(GetWorkingDirectory)(THIS_ LPWSTR pszDir, int cchMaxPath) PURE;
    STDMETHOD(SetWorkingDirectory)(THIS_ const wchar_t* pszDir) PURE;

    STDMETHOD(GetArguments)(THIS_ LPWSTR pszArgs, int cchMaxPath) PURE;
    STDMETHOD(SetArguments)(THIS_ const wchar_t* pszArgs) PURE;
    ...
    STDMETHOD(SetIconLocation)(THIS_ const wchar_t* pszIconPath, int iIcon) PURE;

    STDMETHOD(SetRelativePath)(THIS_ const wchar_t* pszPathRel, unsigned long dwReserved) PURE;

    STDMETHOD(Resolve)(THIS_ HWND hwnd, unsigned long fFlags) PURE;

    STDMETHOD(SetPath)(THIS_ const wchar_t* pszFile) PURE;
};
...
DECLARE_INTERFACE_(INewShortcutHookA, IUnknown) // nshhk
{
    ...
    STDMETHOD(SetReferent)(THIS_ const char* pcszReferent, HWND hwnd) PURE;
    STDMETHOD(GetReferent)(THIS_ LPSTR pszReferent, int cchReferent) PURE;
    STDMETHOD(SetFolder)(THIS_ const char* pcszFolder) PURE;
...
DECLARE_INTERFACE_(INewShortcutHookW, IUnknown) // nshhk
{
    ...
    // *** INewShortcutHook methods ***
    STDMETHOD(SetReferent)(THIS_ const wchar_t* pcszReferent, HWND hwnd) PURE;
    STDMETHOD(GetReferent)(THIS_ LPWSTR pszReferent, int cchReferent) PURE;
    STDMETHOD(SetFolder)(THIS_ const wchar_t* pcszFolder) PURE;
...
DECLARE_INTERFACE_(ICopyHookA, IUnknown)        // sl
{
    ...
    STDMETHOD_(UINT,CopyCallback) (THIS_ HWND hwnd, UINT wFunc, UINT wFlags, const char* pszSrcFile, unsigned long dwSrcAttribs,
                                   const char* pszDestFile, unsigned long dwDestAttribs) PURE;
};
...
DECLARE_INTERFACE_(ICopyHookW, IUnknown)        // sl
{
    ...
    STDMETHOD_(UINT,CopyCallback) (THIS_ HWND hwnd, UINT wFunc, UINT wFlags, const wchar_t* pszSrcFile, unsigned long dwSrcAttribs,
                                   const wchar_t* pszDestFile, unsigned long dwDestAttribs) PURE;
};
...
typedef struct
{
    ...
    unsigned long cbSize;           // Size of structure for future expansion...
...
DECLARE_INTERFACE_(IShellBrowser, IOleWindow)
{
    ...
    STDMETHOD(GetViewStateStream)(THIS_ unsigned long grfMode,
...
//
//  ICommDlgBrowser::OnStateChange(unsigned long uChange)
...
DECLARE_INTERFACE_(IShellView, IOleWindow)
{
    ...
    STDMETHOD(AddPropertySheetPages)(THIS_ unsigned long dwReserved,
...
DECLARE_INTERFACE_(IShellView2, IShellView)
{
    ...
    // *** IShellView2 methods ***
    STDMETHOD(GetView)(THIS_ SHELLVIEWID* pvid, unsigned long uView) PURE;
...
typedef struct _browseinfoA {
    ...
    const char*       lpszTitle;      // text to go in the banner over the tree.
...
typedef struct _browseinfoW {
    ...
    const wchar_t*      lpszTitle;      // text to go in the banner over the tree.
    UINT         ulFlags;       // Flags that control the return stuff
    BFFCALLBACK  lpfn;
    LPARAM      lParam;         // extra info that's passed back in callbacks
...
DECLARE_INTERFACE_(IShellFolder, IUnknown)
{
    ...
    STDMETHOD(ParseDisplayName) (THIS_ HWND hwndOwner,
        ...
        unsigned long * pchEaten, LPITEMIDLIST * ppidl, unsigned long *pdwAttributes) PURE;

    STDMETHOD(EnumObjects) ( THIS_ HWND hwndOwner, unsigned long grfFlags, LPENUMIDLIST * ppenumIDList) PURE;

    STDMETHOD(BindToObject)     (THIS_ LPCITEMIDLIST pidl, LPBC pbcReserved,
                                 REFIID riid, void* * ppvOut) PURE;
    ...
    STDMETHOD(CreateViewObject) (THIS_ HWND hwndOwner, REFIID riid, void* * ppvOut) PURE;
    STDMETHOD(GetAttributesOf)  (THIS_ UINT cidl, LPCITEMIDLIST * apidl,
                                    unsigned long * rgfInOut) PURE;
    STDMETHOD(GetUIObjectOf)    (THIS_ HWND hwndOwner, UINT cidl, LPCITEMIDLIST * apidl,
                                 REFIID riid, UINT * prgfInOut, void* * ppvOut) PURE;
    STDMETHOD(GetDisplayNameOf) (THIS_ LPCITEMIDLIST pidl, unsigned long uFlags, LPSTRRET lpName) PURE;
    STDMETHOD(SetNameOf)        (THIS_ HWND hwndOwner, LPCITEMIDLIST pidl,
                                 LPCOLESTR lpszName, unsigned long uFlags,
...
typedef struct _DROPFILES {
   unsigned long pFiles;                       // offset of file list
...
#define SHCNF_DWORD       0x0003        // unsigned long
