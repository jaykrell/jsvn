...
EXTERN_C typedef HRESULT (__stdcall *LPDAOBINDFUNC)(unsigned long cb, unsigned long dwUser, void* *ppData);
#define DAOBINDINGFUNC(f)   STDAPI f (unsigned long cb, unsigned long dwUser, void* *ppData)

typedef struct
	{
	DAOCOLUMNID     columnID;
	unsigned long           cbDataOffset;
	unsigned long           cbMaxLen;
	unsigned long           cbInfoOffset;
	unsigned long           dwBinding;
...
typedef struct
	{
	unsigned long           cRowsRequested;
	unsigned long           dwFlags;
	void*          pData;
	void*          pVarData;
	unsigned long           cbVarData;
	unsigned long           cRowsReturned;
	} DAOFETCHROWS;
...
DECLARE_INTERFACE_(ICDAORecordset, IDispatch)
	{
	STDMETHOD(GetRows)          (THIS_ long cRowsToSkip, long cCol, LPDAOCOLUMNBINDING prgBndCol, unsigned long cbRowLen, LPDAOFETCHROWS pFetchRows) PURE;
	STDMETHOD(SetNotify)		(THIS_ REFIID riid, BOOL fNotify);
	STDMETHOD(GetNotify)		(THIS_ REFIID riid, BOOL *fNotify);
	STDMETHOD(OnBeforeNotify)	(THIS_ REFIID riid, unsigned long cat, unsigned long rsn, VARIANT v1, VARIANT v2);
	STDMETHOD(OnAfterNotify)	(THIS_ REFIID riid, unsigned long cat, unsigned long rsn, VARIANT v1, VARIANT v2, HRESULT hr);
