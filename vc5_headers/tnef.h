...
#define TNEF_DECODE                 ((unsigned long) 0)
#define TNEF_ENCODE                 ((unsigned long) 2)

#define TNEF_PURE                   ((unsigned long) 0x00010000)
#define TNEF_COMPATIBILITY          ((unsigned long) 0x00020000)
#define TNEF_BEST_DATA              ((unsigned long) 0x00040000)
#define TNEF_COMPONENT_ENCODING     ((unsigned long) 0x80000000)

/*  AddProps, ExtractProps */

#define TNEF_PROP_INCLUDE           ((unsigned long) 0x00000001)
#define TNEF_PROP_EXCLUDE           ((unsigned long) 0x00000002)
#define TNEF_PROP_CONTAINED         ((unsigned long) 0x00000004)
#define TNEF_PROP_MESSAGE_ONLY      ((unsigned long) 0x00000008)
#define TNEF_PROP_ATTACHMENTS_ONLY  ((unsigned long) 0x00000010)
#define TNEF_PROP_CONTAINED_TNEF    ((unsigned long) 0x00000040)

/*  FinishComponent */

#define TNEF_COMPONENT_MESSAGE      ((unsigned long) 0x00001000)
#define TNEF_COMPONENT_ATTACHMENT   ((unsigned long) 0x00002000)

#define MAPI_ITNEF_METHODS(IPURE)                                       \
    MAPIMETHOD(AddProps)                                                \
        (THIS_  unsigned long                       ulFlags,                    \
                unsigned long                       ulElemID,                   \
                void*                      lpvData,                    \
...
typedef HRESULT (__stdcall  * LPOPENTNEFSTREAM) (
    void*              lpvSupport,
    LPSTREAM            lpStream,
    LPTSTR              lpszStreamName,
    unsigned long               ulFlags,
...
typedef HRESULT (__stdcall  * LPOPENTNEFSTREAMEX) (
    void*              lpvSupport,
    LPSTREAM            lpStream,
    LPTSTR              lpszStreamName,
    unsigned long               ulFlags,
...
STDMETHODIMP GetTnefStreamCodepage (
    LPSTREAM            lpStream,
    unsigned long  *         lpulCodepage,
    unsigned long  *         lpulSubCodepage);

typedef HRESULT (__stdcall  * LPGETTNEFSTREAMCODEPAGE) (
    LPSTREAM            lpStream,
    unsigned long  *         lpulCodepage,
    unsigned long  *         lpulSubCodepage);
...
#define MAC_BINARY  ((unsigned long) 0x00000001)
...
#define ATT(_atp, _id)              ((((unsigned long) (_atp)) << 16) | ((WORD) (_id)))
