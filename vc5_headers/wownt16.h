...
unsigned long  PASCAL GetVDMPointer32W(void* vp, UINT fMode);
...
unsigned long  PASCAL LoadLibraryEx32W(const char* lpszLibFile, unsigned long hFile, unsigned long dwFlags);
unsigned long  PASCAL GetProcAddress32W(unsigned long hModule, const char* lpszProc);
unsigned long  PASCAL FreeLibrary32W(unsigned long hLibModule);
...
//
//   unsigned long cParams          - Number of optional unsigned long parameters (0-32)
//
//   void* fAddressConvert - Bit Field, for 16:16 address Convertion. The
...
//
//   unsigned long lpProcAddress   -  32 bit native address to call (use LoadLibraryEx32W
...
//
// unsigned long  PASCAL CallProc32W( unsigned long p1, ... , unsigned long lpProcAddress,
//                                        unsigned long fAddressConvert, unsigned long cParams);
//
// where the value in cParams must match the actual number of optional
// parameters (p1-pn) given AND the "unsigned long p1, ..." must be replaced by
...
// "unsigned long p2, unsigned long p3" instead.  The fAddressConvert parameter uses bit 1
...
unsigned long  CDECL CallProcEx32W( unsigned long, unsigned long, unsigned long, ... );
