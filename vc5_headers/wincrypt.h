...
#define KP_PERMISSIONS          6       // Key permissions unsigned long
...
typedef struct _RSAPUBKEY {
        unsigned long   magic;                  // Has to be RSA1
        unsigned long   bitlen;                 // # of bits in modulus
        unsigned long   pubexp;                 // public exponent
...
CryptAcquireContextW(
    HCRYPTPROV *phProv,
    const wchar_t* pszContainer,
    const wchar_t* pszProvider,
    unsigned long dwProvType,
    unsigned long dwFlags);
#ifdef UNICODE
...
__stdcall
CryptSetProviderA(
    const char* pszProvName,
    unsigned long dwProvType);
/*WINADVAPI*/
BOOL
__stdcall
CryptSetProviderW(
    const wchar_t* pszProvName,
    unsigned long dwProvType);
#ifdef UNICODE
