...
NetAlertRaise(
    IN const wchar_t* AlertEventName,
    IN void*  Buffer,
...
NetAlertRaiseEx(
    IN const wchar_t* AlertEventName,
    IN void*  VariableInfo,
    IN unsigned long   VariableInfoSize,
    IN const wchar_t* ServiceName
    );
...
typedef struct _STD_ALERT {
    unsigned long  alrt_timestamp;
    WCHAR  alrt_eventname[EVLEN + 1];
    WCHAR  alrt_servicename[SNLEN + 1];
}STD_ALERT, *PSTD_ALERT, *LPSTD_ALERT;

typedef struct _ADMIN_OTHER_INFO {
    unsigned long  alrtad_errcode;
    unsigned long  alrtad_numstrings;
}ADMIN_OTHER_INFO, *PADMIN_OTHER_INFO, *LPADMIN_OTHER_INFO;

typedef struct _ERRLOG_OTHER_INFO {
    unsigned long  alrter_errcode;
    unsigned long  alrter_offset;
}ERRLOG_OTHER_INFO, *PERRLOG_OTHER_INFO, *LPERRLOG_OTHER_INFO;

typedef struct _PRINT_OTHER_INFO {
    unsigned long  alrtpr_jobid;
    unsigned long  alrtpr_status;
    unsigned long  alrtpr_submitted;
    unsigned long  alrtpr_size;
}PRINT_OTHER_INFO, *PPRINT_OTHER_INFO, *LPPRINT_OTHER_INFO;

typedef struct _USER_OTHER_INFO {
    unsigned long  alrtus_errcode;
    unsigned long  alrtus_numstrings;
