...
#define dwDiffAT(at1, at2)\
    (1000L*((at2).sec - (at1).sec) - (unsigned long)(at1).ms + (unsigned long)(at2).ms)
...
//---------------------------------------------------------------------------
#ifndef NOPENTARGET
#define HwndFromHtrg(htrg)      ((HWND)(unsigned long)(htrg))
...
// Pointer Types:
typedef ALC *                LPALC;        // ptr to ALC
typedef void*                  LPOEM;        // alias
typedef SYV *                LPSYV;        // ptr to SYV
typedef HPENDATA *           LPHPENDATA;   // ptr to HPENDATA

// Function Prototypes:
typedef int         (__stdcall *ENUMPROC)(LPSYV, int, VOID *);
typedef int         (__stdcall *LPDF)(int, void*, void*, int, unsigned long, unsigned long);
typedef BOOL        (__stdcall *RCYIELDPROC)(VOID);

// Structures:

typedef struct tagABSTIME       // 2.0 absolute date/time
   {
   unsigned long sec;      // number of seconds since 1/1/1970, ret by CRTlib time() fn
...
typedef struct tagCTLINITHEDIT  // 2.0 init struct for (h)edit
   {
   unsigned long cbSize;                // sizeof(CTLINITHEDIT)
   HWND hwnd;                   // (h)edit window handle
   int id;                      // its id
   unsigned long dwFlags;               // CIE_xx
   unsigned long dwReserved;            // for future use
   }
...
typedef struct tagIMECOLORS     // 2.0 IME undetermined string color info.
   ...
   COLORREF  *lprgbIme;      // address of array of RGB values
   }
   IMECOLORS,  *LPIMECOLORS;

typedef struct tagCTLINITBEDIT  // 2.0 init struct for box edit
   {
   unsigned long cbSize;                // sizeof(CTLINITBEDIT)
...
typedef struct tagPENDATAHEADER // 1.0 main pen data header
   ...
   unsigned long rgbInk;                // ink color
   }
...
typedef struct tagSTROKEINFO    // 1.0 stroke header
   ...
   unsigned long dwTick;                // time at beginning of stroke
   }
   STROKEINFO,  *LPSTROKEINFO;

typedef struct tagPENTIP        // 2.0 Pen Tip characteristics
    {
    unsigned long cbSize;               // sizeof(PENTIP)
    ...
    unsigned long dwFlags;              // TIP_xx flags
    unsigned long dwReserved;           // for future expansion
    }
    PENTIP,  *LPPENTIP;

typedef BOOL (__stdcall *ANIMATEPROC)(HPENDATA, UINT, UINT, UINT *, LPARAM);

typedef struct tagANIMATEINFO   // 2.0 Animation parameters
    {
    unsigned long cbSize;               // sizeof(ANIMATEINFO)
...
typedef BOOL (__stdcall *LPFNRAWHOOK)(LPPENPACKET);
#endif /*!NOPENDRIVER */
...
typedef BOOL (__stdcall *HRCRESULTHOOKPROC)
    (HREC, HRC, UINT, UINT, UINT, void*);

// Inksets:
DECLARE_HANDLE(HINKSET);        // handle to an inkset
typedef HINKSET                 * LPHINKSET; // ptr to HINKSET
...
typedef struct tagCTLINITIEDIT  // 2.0 init struct for Ink Edit
    {
    unsigned long cbSize;               // sizeof(CTLINITIEDIT)
    ...
    unsigned long dwApp;                // application-defined data
...
typedef struct tagPDEVENT       // 2.0
    {
    unsigned long cbSize;               // sizeof(PDEVENT)
...
typedef struct tagPCMINFO       // 2.0 Pen Collection Mode Information
   {
   unsigned long cbSize;                // sizeof(PCMINFO)
   unsigned long dwPcm;                 // PCM_xxx flags
   ...
   unsigned long dwTimeout;             // if finish after timeout, this many ms
   }
    PCMINFO,  *LPPCMINFO;

typedef struct tagINKINGINFO    // 2.0 Pen Inking Information
   {
   unsigned long cbSize;                // sizeof(INKINGINFO)
...
typedef struct tagRC            // 1.0 Recognition Context (RC)
   ...
   unsigned long dwAppParam;            // for application use
   unsigned long dwDictParam;           // for app use to be passed on to dictionaries
   unsigned long dwRecognizer;          // for app use to be passed on to recognizer
...
typedef int         (__stdcall *LPFUNCRESULTS)(LPRCRESULT, REC);
...
typedef struct tagTARGET        // 2.0 Geometry for a single target.
   {
   unsigned long dwFlags;               // individual target flags
   unsigned long idTarget;              // TARGINFO.rgTarget[] index
   HTRG  htrgTarget;            // HANDLE32 equiv
   RECTL rectBound;             // Bounding rect of the target
   unsigned long dwData;                // data collection info per target
...
typedef struct tagTARGINFO      // 2.0 A set of targets
   {
   unsigned long cbSize;                // sizeof(TARGINFO)
...
typedef struct tagINPPARAMS     // 2.0
   {
   unsigned long cbSize;                // sizeof(INPPARAMS)
...
LRESULT __stdcall    DefPenWindowProc(HWND, UINT, WPARAM, LPARAM);
...
BOOL      WINAPI GetPenDataStroke(LPPENDATA, UINT, LPPOINT *,
                                  void* *, LPSTROKEINFO);
#endif //!NOPENAPIFUN
HPENDATA  WINAPI AddPointsPenData(HPENDATA, LPPOINT, void*, LPSTROKEINFO);
int       WINAPI CompressPenData(HPENDATA, UINT, unsigned long);
...
int       WINAPI ExtractPenDataStrokes(HPENDATA, UINT, LPARAM,
                                       LPHPENDATA, UINT);
int       WINAPI GetPenDataAttributes(HPENDATA, void*, UINT);
BOOL      WINAPI GetPenDataInfo(HPENDATA, LPPENDATAHEADER, LPPENINFO, unsigned long);
BOOL      WINAPI GetPointsFromPenData(HPENDATA, UINT, UINT, UINT, LPPOINT);
int       WINAPI GetStrokeAttributes(HPENDATA, UINT, void*, UINT);
int       WINAPI GetStrokeTableAttributes(HPENDATA, UINT, void*, UINT);
...
int       WINAPI InsertPenDataPoints(HPENDATA, UINT, UINT, UINT,
                                     LPPOINT, void*);
int       WINAPI InsertPenDataStroke(HPENDATA, UINT, LPPOINT, void*,
                                     LPSTROKEINFO);
...
int       WINAPI TrimPenData(HPENDATA, unsigned long, unsigned long);
...
REC       WINAPI GetPenHwData(LPPOINT, void*, int, UINT, LPSTROKEINFO);
REC       WINAPI GetPenHwEventData(UINT, UINT, LPPOINT, void*,
                                   int, LPSTROKEINFO);
...
int       WINAPI AddPenInputHRC(HRC, LPPOINT, void*, UINT, LPSTROKEINFO);
...
HWL       WINAPI CreateHWL(HREC, LPSTR, UINT, unsigned long);
...
int       WINAPI GetInternationalHRC(HRC, UINT *, LPSTR, UINT *,
                                     UINT *);
...
int       WINAPI ProcessHRC(HRC, unsigned long);
...
int       WINAPI SetInternationalHRC(HRC, UINT, const char*, UINT, UINT);
...
int       WINAPI GetPenInput(HPCM, LPPOINT, void*, UINT, UINT, LPSTROKEINFO);
int       WINAPI PeekPenInput(HPCM, UINT, LPPOINT, void*, UINT);
...
BOOL      WINAPI CorrectWriting(HWND, LPSTR, UINT, void*, unsigned long, unsigned long);
#ifndef _WIN32
...
UINT      WINAPI EnumSymbols(LPSYG, UINT, ENUMPROC, void*);
...
// Ink Targeting:
int       WINAPI TargetPoints(LPTARGINFO, LPPOINT, unsigned long, UINT, LPSTROKEINFO);
